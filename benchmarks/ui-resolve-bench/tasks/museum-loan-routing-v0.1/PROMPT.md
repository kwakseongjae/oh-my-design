# Task: improve the Folio museum-loan handoff

Work only inside this repository. Improve the existing `index.html` into a
clear, production-ready museum-loan routing handoff for Folio, a fictional
collections operations tool.
...
condition, insurance, transport, schedule, custody, or exhibition claims.

Keep every supplied object and destination identifier, every `data-bench` hook,
...
1. choose condition-first routing, calendar grouping, or manual assignment;
2. turn source-note preservation on and off with correct `aria-pressed`;
3. reject an empty handoff label, focus the field, then accept
   `Autumn gallery loan` and announce the changed state;
...
5. preserve exact identifiers and the relationship between routing policy,
   object, destination, registrar, source-note setting, handoff label, and
   final review action without injected break opportunities, mid-token
...
separators into identifiers. Do not turn each object into a decorative card or
invent condition grades, valuations, shipping dates, staff counts, visitor
proof, or loan outcomes.
