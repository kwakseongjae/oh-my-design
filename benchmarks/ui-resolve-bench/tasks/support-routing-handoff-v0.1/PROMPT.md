# Task: improve the Switchboard support-routing handoff

Work only inside this repository. Improve the existing `index.html` into a
clear, production-ready support-routing handoff surface for Switchboard, a
fictional support operations tool.
...
staffing, ticket, response-time, customer, availability, or service claims.

Keep every supplied queue and destination identifier, every `data-bench` hook,
...
1. choose follow-the-sun, regional ownership, or manual assignment;
2. turn original-assignee context preservation on and off with correct `aria-pressed`;
3. reject an empty handoff label, focus the field, then accept
   `Weekend support handoff` and announce the changed state;
...
5. preserve exact identifiers and the relationship between routing policy,
   source queue, destination, owner, context setting, handoff label, and final
...
separators into identifiers. Do not turn each queue into a decorative card or
invent staffing levels, response times, resolved tickets, customer proof, or
handoff outcomes.
