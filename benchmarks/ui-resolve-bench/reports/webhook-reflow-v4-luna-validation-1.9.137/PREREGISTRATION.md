# Webhook destination reflow v4 Luna validation — preregistration

Status: **LOCKED before preparation and provider execution**.

- Task: exact unseen `webhook-destination-routing-v0.1` `0.1.0`
- Arms: exact previous canonical `omd-portable-jade` versus exact v4
  `omd-portable-reflow-v4-candidate`
...
- Attribution: Internal provider-scoped result, not merged with prior Grok or
  Luna task families
...
generated-label, scroll-focus, or residual-width regression. A provider or
controller failure freezes this root independently.
