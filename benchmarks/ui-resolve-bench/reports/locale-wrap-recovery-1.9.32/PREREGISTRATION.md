# Locale wrapped-control recovery — 1.9.32

Locked before workspace preparation or provider generation on 2026-07-23.

## Question

Can the OmD locale stack produce a fully resolved five-locale handoff when the
provider keeps zero browser authority and statically prevents wrapped control
hit-area overlap?

This is a fresh candidate-only recovery for failed 1.9.30 after the
provider-free 1.9.31 task/activation calibration. It does not reuse, resume,
retry, score, or edit `/tmp/u1930`, and it does not change that frozen
decision.

## Frozen basis

- Task: `locale-cli-handoff-v0.1` version `0.3.0`
- Concept/wrap source basis: `0991a69`
- Direct-browser counter/gate basis: `9df4eca`
- Zero-browser authority basis: `2249736`
...
- provider verification is static source plus protected fact, terminology,
  register, repetition, and wrapped-control geometry reasoning;
...
- every bounded required concept and forbidden locale pattern passes;
- command, 12, 3, and `DESIGN.md` preserved in all five locales;
- all four responsive geometry profiles pass;
- no clipped or overlapping control hit areas at 200% zoom;
...
validates this one execution path and may unlock a repeated locale matrix or
the minimum public benchmark UX evidence contract; it does not establish
general locale lift, efficiency, or public frontier status.
