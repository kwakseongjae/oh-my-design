...
This is the fresh operational replacement permitted after 1.9.51 stopped on
Cursor Provider `resource_exhausted`. With Cursor Agent and Composer 2.5 fixed,
the 1.9.49 foreground closure should let the reviewed OmD apply skill resolve
the pricing task in 3/3 trials, avoid every paired objective loss versus Raw
DESIGN.md, and preserve accessibility and Evidence & Unknown.

The only candidate delta from the rejected 1.9.48 matrix is the canonical OmD
skill foreground closure accepted locally in 1.9.49 and in one fresh Composer
cell in 1.9.50. Evaluator schema `0.5`, suite `ui-resolve-v0.2`, task, starter,
Raw DESIGN.md, order, timeout, concurrency, and acceptance are unchanged.
There is no candidate, task, evaluator, prompt, order, or acceptance delta from
1.9.51; only the fresh root and experiment identity change. Every 1.9.48,
1.9.50, and 1.9.51 artifact is outside this denominator.
...
- balanced order: unchanged from 1.9.48;
- root: `/tmp/u1952`.
...
Pass only when all 9 cells complete; OmD is automated-gate and
