...
2f. **`proof execution close latch`로 끝난 증명을 다시 열지 않는다.** 품질 gate는 유지하고 아래 state를 같은 consumer route의 acceptance까지 유지한다.
   ...
   - **준비된 mechanism은 shipped runner를 exact named consumer connection에서 실행하는 단일 command다.** `<current-skill-dir>`와 locked product path만 치환해 `OMD_REFLOW_ARTIFACT=.omd/reflow-closure.json OMD_REFLOW_PRODUCT=<locked-product-path> OMD_REFLOW_HELPER=<current-skill-dir>/scripts/reflow-artifact.mjs browser-harness < <current-skill-dir>/scripts/reflow-browser.py`를 정확히 한 번 실행한다. browser 측정 Python을 즉석 작성하거나 `browser-harness --help`, `browser-harness skill`, `command -v`, socket/port 탐색을 먼저 실행하지 않는다. shipped runner가 환경의 `BU_NAME`과 `BU_CDP_URL`을 그대로 읽어 existing consumer Chrome에 attach하고 `{390, zoom 1}`·`{320, zoom 1}`·`{640, zoom 2}`의 모든 registered selector를 측정해 같은 process에서 terminal finalize한다. heredoc, `chromium.launch`, `launch_persistent_context`, Chrome process 생성, 다른 browser/port fallback은 사용하지 않는다. helper stdout의 terminal marker 외에 별도 print/read/finalize를 붙이지 않으며 command 반환 뒤 artifact `rg`/`sed`/`cat`을 실행하지 않는다. exact attach/실행 infrastructure error는 runner가 즉시 `unresolved`로 잠그고 recovery를 시도하지 않는다.
