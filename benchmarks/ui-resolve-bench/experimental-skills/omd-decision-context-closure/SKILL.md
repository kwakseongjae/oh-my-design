...
2f. **`proof execution close latch`로 끝난 증명을 다시 열지 않는다.** 품질 gate는 유지하고 아래 state를 같은 consumer route의 acceptance까지 유지한다.
   ...
   - **준비된 mechanism은 exact named consumer connection을 쓰는 `browser-harness <<'PY' … PY`다.** 사용법을 알아내기 위한 `browser-harness --help`, `browser-harness skill`, `command -v`, socket/port 탐색을 먼저 실행하지 않는다. 환경의 `BU_NAME`과 `BU_CDP_URL`을 그대로 읽어 existing consumer Chrome에 attach하며, heredoc 안팎에서 `chromium.launch`, `launch_persistent_context`, Chrome process 생성 또는 다른 browser/port fallback을 사용하지 않는다. 첫 browser-harness 호출 하나 안에서 실제 route를 열고, states와 `{390, zoom 1}`·`{320, zoom 1}`·`{640, zoom 2}`를 순회하며 모든 registered selector의 character-range line tops·computed typography·inline reserve·overflow·cardinality·association과 document/body widths를 수집한다. 각 조건은 reload 뒤 document zoom을 적용하고 computed zoom을 관찰한다. 그 Python process가 artifact에 exact connection identity와 `attached_existing`, `launched_browser: false`를 쓰고 `subprocess.run(["node", helper, "finalize", artifact], check=True)`까지 실행한다. helper가 stdout terminal marker를 자동 출력하므로 별도 print/read/finalize를 붙이지 않는다. browser command가 반환된 뒤 `rg`/`sed`/`cat`/helper를 실행하지 않는다. exact attach/실행 infrastructure error면 같은 process에서 즉시 `unresolved`로 잠그며 recovery나 새 browser launch를 실행하지 않는다.
