...
2e. **`reflow-integrity closure`는 compact group packet 하나로 실행한다.** 같은 consumer route의 390px·320px·actual 200% reflow를 검사한다. 200%는 640px viewport만 뜻하지 않는다. `viewport_width: 640`과 `document.documentElement.style.zoom = "2"`를 함께 적용해 effective CSS width 320px 조건을 만든다. 첫 CSS 편집 전에 필요한 source inspection을 전부 끝내고 **`.omd/reflow-closure.json`에 schema `0.3` 초안을 실제 저장**한다. `acceptance_sequence.source_inspection_complete: true`는 이후 제품 source를 `rg`/`sed`/`awk`로 다시 읽지 않겠다는 latch다. 같은 selector·역할·longest value를 공유하는 반복 행은 인스턴스마다 복제하지 않고 `row_groups.expected_count`로 전부 계상한다. 인접한 의미 관계가 다른 carrier는 합치지 않는다.
   ...
   reflow_work_packet:
     ...
     acceptance_debt_ledger:
       - id: "one supplied or measured failure"
         ...
         static_guardrail: { required_literals: ["manifest-bound correction when applicable"], forbidden_literals: ["manifest-bound bad literal when applicable"], forbidden_patterns: ["manifest-bound bad source pattern when applicable"] }
   ...
   1. **INVENTORY.** `row_groups`에는 one-line 계약이 있는 visible atomic identifier, 선택 target/source/artifact filename, short control label, 그리고 **측정 시작 state에서 non-empty로 보이는** dynamic state/status만 넣는다. evidence·summary·metadata·supplied-count는 rendered text가 48자 이하이고 제품 계약이 명시적으로 one-line을 요구할 때만 atomic row다. 그 밖의 evidence 문장·일반 heading/body prose·현재 비어 있거나 hidden인 status는 row가 아니라 carrier 안의 보존 콘텐츠로 남긴다. **row selector 자체의 rendered text/value가 `longest_value`와 정확히 대응해야 한다.** 여러 unrelated descendant를 포함하는 card/container나 외부 label의 이름을 대신 적은 empty input을 row로 등록하지 않고, 그 atomic text를 직접 소유하는 가장 작은 stable selector를 사용한다. snapshot-backed row selector는 제품 편집 전에 실제 존재하는 class, id, 또는 attribute anchor를 포함해야 한다. 제품 edit에서 새로 붙일 `.event-log-form` 같은 class를 pre-edit selector로 미리 등록하거나 tag-only selector를 쓰지 않는다. `lock`이 selector provenance를 거부하면 제품을 아직 편집하지 않은 채 기존 pre-edit hook으로 artifact를 고쳐 다시 lock한다. 같은 selector/role의 반복은 `expected_count`로 묶되 측정 시작 state에서 실제 렌더되는 값 중 가장 긴 값을 `longest_value`로 기록한다. 하나의 protected wrapper에 `ID-A + ID-B`처럼 복수 exact token이 있으면 ordered `atomic_parts`와 `line_contract: parent-one-line`을 반드시 기록한다. 단일 atomic value는 `line_contract: single-token`이다. paired toggle/button/select copy는 tag와 무관하게 `control-label`이다. 각 group을 소유하거나 다른 내용과의 관계를 전달하는 protected/named scope를 `carriers`에 전부 등록한다. `static_closure_manifest`에는 제품 편집 전에 이미 확인한 product path, 반드시 남을 literal, 사라져야 할 literal/pattern, 정확한 hook/cardinality를 모두 선언한다. 패턴과 count를 편집 뒤 새로 발명하지 않는다. `lock` helper는 편집 전 product source와 sha256을 artifact 안에 자동 snapshot한다. 모델은 CSS shorthand나 inherited unitless line-height를 계산해 숫자로 옮기지 않고 모든 row에 `typography_contract: { source: deterministic-pre-edit-snapshot }`을 쓴다. `lock`이 성공하면 즉시 제품 편집으로 넘어가며 helper source나 hash 알고리즘을 읽지 않는다.
   2. **FIT.** shipped runner가 같은 browser command 안에서 helper가 잠근 pre-edit snapshot과 편집 후 product를 동일 조건으로 각각 렌더해 각 group의 computed font size·line height·weight를 exact 비교한다. 따라서 `font: 14px/1.55` 아래의 18px target처럼 unitless line-height가 자식 font size에 다시 계산되는 경우도 모델 추정값이 아니라 실제 27.9px로 잠긴다. DESIGN.md type role과 target emphasis를 보존하고 더 작은 임의 type, 축약, `clamp()` 하한으로 맞추지 않는다. pass는 모든 조건에서 snapshot typography가 exact하고 comparison-scroll이 아닌 각 row에 최소 **8 CSS px의 측정된 inline reserve**가 남을 때만 가능하다. 단, 첫 edit 계획은 각 non-scroll row에 `planned_fit_reserve_css_px: 16`을 잡는다. 경계 8px에 맞추는 계획은 font rendering·scrollbar·zoom 차이를 흡수하지 못하므로 lock에서 거절한다. 8px는 새 시각 token이 아니라 engine/rounding 차이에서 zero-edge pass를 막는 검증 안전 여유다. source-only 또는 경계에 딱 맞는 결과는 `unresolved`다. `viewport → page inset → card padding → section inset → reading width → carrier full-row → carrier stack/relocate` 순서로 연속 폭을 회수한다. 이 순서를 거치기 전에 atomic text에 scroll을 주지 않는다.
   3. **REFLOW.** 가장 좁은 조건에서 group의 longest atomic child와 padding/gap을 판단한다. fit하지 않으면 text를 깨지 말고 parent row를 `full-row`, 다음으로 `stack`한다. compound wrapper는 protected selector·accessible text·원문 순서를 유지하고 각 `atomic_parts`만 관측 가능한 child span으로 감싼다. **wrapper 자체에 one-line 계약이 있으면 `atomic_parts`는 separator wrap 허가가 아니다.** parts와 separator 전체를 한 atomic group으로 유지하고, fit하지 않으면 carrier를 full-row/stack/relocate해 폭을 회수한다. mobile cascade에서 desktop track·basis·min-width를 해제하고 필요한 child에 `min-width: 0`을 둔다. 그래도 물리적으로 안 맞고 shared header·legend가 의미 관계를 제공할 때만 row selector와 다른 **target-only 관계 carrier**를 named `comparison-scroll`로 쓴다. 이 carrier는 `carriers`에 별도로 등록하고 `binds_row_groups`에는 그 compound row 하나만 둔다. target-only carrier 바깥에 evidence·state·action을 두어 초기 scroll 위치에서도 action이 잘리거나 사라지지 않게 한다. carrier는 exact accessible name, `tabindex="0"`, visible `:focus-visible`을 갖고 `scroll_contract`에 기록한다. runner는 허용되지 않은 carrier overflow, comparison carrier 안의 다른 registered row·focusable descendant, 초기 위치에서 잘린 focusable control을 실패 처리한다. protected target/identifier/state 같은 passive text 자체의 computed overflow가 `auto|scroll`이면 geometry가 맞아도 실패다. stack은 기존 carrier 자체를 relocate한다. `display:none` 뒤 generated content·`data-*`·aria-label·hook 없는 span 복제, passive text scroller, word-break, token 내부 break character, generated separator는 실패다.
...
2f. **`proof execution close latch`로 끝난 증명을 다시 열지 않는다.** 품질 gate는 유지하고 아래 state를 같은 consumer route의 acceptance까지 유지한다.
   ...
   - pre-edit 한 번에 token·hook/cardinality·state source·responsive risk·reflow rows와 정확한 edit 위치를 inventory하고 `source_inspection_complete: true`, `inventory: closed`로 잠근다. **첫 product edit 뒤에는 그 edit이 마지막인지 확신이 없어도** 제품 source에 대한 `rg`/`sed`/`awk`/`wc`/diff read를 실행하지 않는다. 불확실하면 edit 전에 더 읽고, latch 뒤에는 한 planned patch transaction만 수행한다.
   - 제품 edit을 한 transaction으로 묶어 `revision`을 1 올리고 `product_edit: changed`, 두 proof state를 `open`으로 만든다. product edit 뒤 첫 shell command는 `node <current-skill-dir>/scripts/reflow-artifact.mjs static-close .omd/reflow-closure.json <locked-product-path>` 한 번뿐이다. 이 결정론 helper가 pre-edit `static_closure_manifest`의 syntax-independent literal 존재/부재, hook cardinality, supplied fact, forbidden text hack을 제품 파일 한 번의 read로 함께 닫는다. `lock` stdout의 `static_edit_guardrails`를 edit payload의 부정 계약으로 취급한다. `forbidden_patterns`가 CSS property를 가리키면 그 선언은 **완전히 삭제**해야 한다. `word-break: normal`, `initial`, `unset`, `revert`, `inherit`처럼 중립값으로 바꾸는 것도 pattern 존재 자체를 위반하므로 넣지 않는다. patch를 적용하기 전 변경 payload에 required/forbidden/count 계약을 한 번 대조하고, 금지된 property name이 payload에 있으면 shell 검증으로 넘어가지 말고 payload에서 제거한다. model이 post-edit `node - <<`, inline JS, `rg`/`sed`/`awk`/`wc`, 임시 verifier 또는 ad-hoc regex static check를 작성하거나 실행하지 않는다. helper 전 일부 확인 명령도 범위와 결과에 관계없이 이미 static budget을 소비한 것이며, 이후 수정으로 revision을 올려도 task-level proof compliance는 복구되지 않는다. helper가 red면 exactly-once static budget이 소비된다. 안전한 제품 수정은 허용하지만 helper를 고쳐서 다시 실행하지 않고 이번 run은 `proof noncompliant`로 전달한 뒤 terminal browser accounting만 정직하게 계속한다.
   ...
   - **준비된 mechanism은 shipped runner를 exact named consumer connection에서 실행하는 단일 command다.** `<current-skill-dir>`와 locked product path만 치환해 `OMD_REFLOW_ARTIFACT=.omd/reflow-closure.json OMD_REFLOW_PRODUCT=<locked-product-path> OMD_REFLOW_HELPER=<current-skill-dir>/scripts/reflow-artifact.mjs browser-harness < <current-skill-dir>/scripts/reflow-browser.py`를 정확히 한 번 실행한다. browser 측정 Python을 즉석 작성하거나 `browser-harness --help`, `browser-harness skill`, `command -v`, socket/port 탐색을 먼저 실행하지 않는다. shipped runner가 환경의 exact `BU_NAME` named socket으로 existing consumer Chrome에 attach한다. raw `BU_CDP_URL`/`BU_CDP_WS`는 공개된 경우에만 metadata로 사용하고 없더라도 socket attachment를 중단하지 않는다. runner는 `{390, zoom 1}`·`{320, zoom 1}`·`{640, zoom 2}`의 모든 registered selector를 측정해 같은 process에서 terminal finalize한다. heredoc, `chromium.launch`, `launch_persistent_context`, Chrome process 생성, 다른 browser/port fallback은 사용하지 않는다. helper stdout의 terminal marker 외에 별도 print/read/finalize를 붙이지 않으며 command 반환 뒤 artifact `rg`/`sed`/`cat`을 실행하지 않는다. exact attach/실행 infrastructure error는 runner가 즉시 `unresolved`로 잠그고 recovery를 시도하지 않는다.
   - **검증 예산은 `task 전체 deterministic static-close helper 1회 + task 전체 browser-harness command 1회`다.** 첫 product edit 뒤 첫 shell command 하나가 static closure 전체다. manifest에 빠진 검사를 채우려고 model-authored command를 추가하지 않는다. 범위가 다르거나 helper가 red여도 두 번째 shell command는 duplicate static closure다. source를 보기 위한 `sed`/`rg`/`awk`/`wc`/diff도 제품 diff 뒤 실행하면 static closure로 소비된다. 따라서 **첫 product edit 전** 필요한 targeted read와 manifest 선언을 끝내고, edit 뒤에는 `static-close` 한 번과 준비된 browser command 한 번만 실행한다. browser command 안의 `finalize`는 `static_closure.state: passed`, `attempts: 1`일 때만 성공할 수 있다. browser 결과가 red여도 제품을 다시 고치지 않으면 `ready`로 잠그고 unresolved를 전달한다.
