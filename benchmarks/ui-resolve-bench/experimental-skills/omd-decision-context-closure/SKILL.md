...
2e. **`reflow-integrity closure`를 다음 순서로 수행한다.** 같은 consumer route의 390px·320px·200% reflow를 모두 검사하며, table/list row뿐 아니라 같은 decision context의 선택 target, source filename, artifact ID까지 포함한다. 순서를 건너뛰거나 뒤 단계의 요령으로 앞 단계 실패를 숨기지 않는다.
   ...
   1. **type role을 먼저 확정한다.** atomic identifier, 제품 계약이 보존하는 짧은 control/state label, 짧은 evidence/summary/metadata/supplied-count line 각각의 선언된 DESIGN.md type role을 기록한다. compact control label은 body/heading 상속값으로 두지 않고 선언된 label role을 적용한다. 짧은 evidence/summary metadata도 body/heading 상속값으로 두지 않고 DESIGN.md에 선언된 label/metadata role이 있으면 그 role을 적용한다. 호환되는 label/metadata role이 선언되지 않았다면 현재 role을 보존하고 더 작은 값을 추측하지 않는다. 선언된 type role 미만 축소, `clamp()`의 더 작은 하한, 임의 축약으로 한 줄을 만들지 않는다. 한 줄 보존은 acceptance 결과이지 첫 CSS 수단이 아니다.
   ...
   closure는 `mid_token_fragmentation: 0`, `short_control_wrap: 0`, `generated_label_overflow: 0`, `reflow_horizontal_overflow: 0`, `reflow_clipped_control: 0`, `decision_role_escape: 0`, `target_emphasis_loss: 0`, `injected_break_opportunity: 0`, `residual_mobile_column_width: 0`, `atomic_context_wrap: 0`, `unfocusable_scroll_region: 0`, `compact_control_label_wrap: 0`, `single_text_scroller: 0`, `undeclared_type_role_shrink: 0`, `undeclared_evidence_type_role: 0`, `full_row_before_text_workaround: 0`, `unresolved_inline_size_budget: 0`이 모두 성립해야 끝난다.
