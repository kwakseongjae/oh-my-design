...
2e. **`reflow-integrity closure`를 다음 순서로 수행한다.** 같은 consumer route의 390px·320px·200% reflow를 모두 검사하며, table/list row뿐 아니라 같은 decision context의 선택 target, source filename, artifact ID까지 포함한다. 순서를 건너뛰거나 뒤 단계의 요령으로 앞 단계 실패를 숨기지 않는다.
   ...
   0. **첫 CSS 편집 전에 `reflow scope manifest`를 고정한다.** 같은 decision context 안에서 one-line 계약을 가진 visible atomic identifier, target/source/artifact filename, evidence/summary/metadata/supplied-count line, short control label, short dynamic state/status output, relational compact-control naming copy를 DOM과 state-rendering source에서 전수한다. 초기 문자열이 비어 있는 live region은 제외하되, JS render 함수·template literal·state map이 실제 선택/토글 상태를 요약해 visible node에 쓰는 copy는 초기 화면 문자열만 보지 말고 dynamic state row로 반드시 포함한다. relational compact-control copy도 `label` tag 여부가 아니라 paired toggle/button/select를 이름 붙이는 관계를 확인해 별도 row로 넣는다. 각 row는 `identity`, `source_selector`, `origin(static|dynamic)`, `semantic_role`, `one_line_contract_source`, `declared_type_role`, `current_controlling_selector`, `planned_final_selector`를 가진다. 일반 display heading/body prose는 명시적 one-line 계약이 없으면 manifest에 넣지 않는다. manifest row를 편집 중 합치거나 삭제하지 말고 마지막 outcome table과 identity/cardinality를 1:1 대조한다. 구체적인 row table을 만들지 않았거나 dynamic state/relational control row를 source에서 확인하지 않았다면 CSS 편집을 시작하지 않고, 행 누락 상태에서 reflow 완료를 주장하거나 final response를 시작하지 않는다.
   1. **type role을 먼저 확정한다.** atomic identifier, 제품 계약이 보존하는 짧은 control/state label, 짧은 evidence/summary/metadata/supplied-count line 각각의 선언된 DESIGN.md type role을 기록한다. compact control label은 element tag가 아니라 같은 control group에서 paired toggle/button/select의 의미를 이름 붙이는 짧은 visible copy로 판정한다. 따라서 source가 `label`, `strong`, `span`, `p` 중 무엇이든 이 관계가 확인되면 body/heading 상속값으로 두지 않고 선언된 label role을 적용한다. 짧은 evidence/summary metadata도 body/heading 상속값으로 두지 않고 DESIGN.md에 선언된 label/metadata role이 있으면 그 role을 적용한다. selected target, source filename, artifact ID와 다른 atomic identifier는 evidence/metadata가 아니며 label/metadata role로 demote하지 않고 선언된 target/identifier role과 emphasis를 보존한다. control 관계나 호환되는 label/metadata role이 선언되지 않았다면 현재 role을 보존하고 더 작은 값을 추측하지 않는다. 선언된 type role 미만 축소, `clamp()`의 더 작은 하한, 임의 축약으로 한 줄을 만들지 않는다. 한 줄 보존은 acceptance 결과이지 첫 CSS 수단이 아니다.
   2. **공간을 먼저 회수한다.** 값을 full-width reading row 또는 충분한 `minmax` 영역에 배치하고 경쟁 column을 접는다. `width: 100%`라는 선언만으로 full row라고 판단하지 않는다. 짧은 atomic evidence/state/control label마다 `viewport_inline_size → page inset → card border/padding → section inset → reading_inline_size → required_inline_size` 순서의 `inline-size budget ledger`를 한 줄로 기록한다. required inline size는 가능하면 같은 type role의 measured text/control width이며, browser proof가 없으면 source에 남은 각 ancestor inset과 fixed track을 합산해 unresolved budget을 숨기지 않는다. `reading_inline_size < required_inline_size`이면 text node를 건드리기 전에 가장 바깥쪽의 task-불필요한 inset/chrome부터 DESIGN.md의 더 작은 선언 spacing token으로 줄이고, 그 chrome이 제품 계약이 아니며 더 작은 token도 없으면 그 reflow viewport에서 0으로 제거한다. inner padding 하나만 줄이고 page/card/section ancestor inset을 남긴 채 full row를 회수했다고 주장하지 않는다. 그 뒤에도 짧은 label이 감싸지면 중첩 wrapper의 불필요한 좌우 margin·padding·fixed track·card chrome을 같은 방식으로 계속 줄인다. full row를 회수하지 않은 채 wrap·nowrap·font 축소로 넘어가지 않는다. 짧은 label이 compact control이나 toggle과 한 row에서 line budget을 넘기면 label에 full-width row를 먼저 주고 control을 다음 row 또는 별도 정렬 영역에 둔다. CSS generated mobile label은 `auto`/`max-content` track 또는 자신의 full-width row를 사용한다. fixed label track은 모든 reflow viewport에서 measured required width 이상일 때만 허용한다.
   ...
   5a. **one-line authority를 final selector에서 실현한다.** scope manifest에서 one-line 계약이 있다고 기록한 row는 공간 회수·구조 reflow·type role 확정 뒤 `final_controlling_selector`의 실제 cascade까지 다시 연다. 그 selector의 종료 상태는 세 가지뿐이다: `(a)` 390/320/200%에서 measured line count 1이고 overflow/clipping 0, `(b)` full-width budget과 선언 type role을 지킨 뒤 selector-level `white-space: nowrap`을 명시하고 같은 세 viewport에서 longest static/dynamic string의 required width가 available width 이내임을 기존 측정 증거로 확인, `(c)` `unresolved`. 자연스럽게 한 줄일 것이라는 기대, 작은 font를 적용했다는 사실, 부모의 `width:100%`, 초기 state 문자열 하나만 본 계산은 selector-level decision이 아니다. source-only 환경에는 새로운 text-width 추정으로 `(a)`나 `(b)`를 만들지 않는다. 기존 measured width가 없으면 안전한 no-wrap의 overflow/clipping을 증명할 수 없으므로 `(c)`로 남긴다. 반대로 one-line row를 manifest에 넣고도 final selector의 wrap 정책을 생략한 채 `pass`로 닫는 것은 실패다. dynamic row는 render function/template/state map의 가능한 문자열 중 가장 긴 값까지 같은 selector로 대조한다. 한 줄을 만들기 위해 카피·identifier·상태 facts를 축약하거나 삭제하지 않는다.
   6. **reflow outcome table로 닫는다.** pre-edit scope manifest의 모든 identity를 같은 cardinality로 가져와 one-line 계약을 가진 atomic identifier, short control/state label, evidence/summary/metadata/supplied-count line, relational compact-control copy마다 한 행씩 기록한다. 일반 display heading이나 body prose는 제품 계약이 one-line을 요구하지 않는 한 이 표에 넣지 않고, 점수를 위해 축소·nowrap·축약·rewrite하지 않는다. 각 행은 `identity`, `semantic_role`, `declared_type_role(source)`, `final_controlling_selector`, `required_inline_size`, `available_inline_size(390|320|200%)`, `line_count(390|320|200%)`, `proof(measured|source-derived|unresolved)`, `status(pass|unresolved)`를 가진다. browser proof가 있으면 실제 computed type·폭·line count를 사용하고, 없으면 source의 모든 ancestor inset/fixed track과 selector specificity로 available width를 유도하되 required width나 line count를 추정해 pass로 바꾸지 않는다. viewport 자체의 폭, `width:100%`, page horizontal overflow 0, screenshot 육안, “검증했다”는 서술은 행 단위 proof가 아니다. manifest와 outcome identity set 또는 row count가 다르면 누락된 행을 자동 pass로 간주하지 않고 closure를 실패시킨다. 모든 scoped 행이 `pass`이고 unresolved 행이 0일 때만 reflow 검증 완료를 보고한다. 하나라도 측정되지 않았거나 감싸면 완료를 주장하지 않고 해당 행과 원인을 `unresolved`로 전달한다.

   closure는 `mid_token_fragmentation: 0`, `short_control_wrap: 0`, `generated_label_overflow: 0`, `reflow_horizontal_overflow: 0`, `reflow_clipped_control: 0`, `decision_role_escape: 0`, `target_emphasis_loss: 0`, `injected_break_opportunity: 0`, `residual_mobile_column_width: 0`, `atomic_context_wrap: 0`, `unfocusable_scroll_region: 0`, `compact_control_label_wrap: 0`, `unclassified_compact_control_copy: 0`, `unmanifested_dynamic_state_row: 0`, `unmanifested_relational_control_copy: 0`, `scope_manifest_row_loss: 0`, `implicit_one_line_selector: 0`, `one_line_authority_without_realization: 0`, `source_claim_without_selector_decision: 0`, `single_text_scroller: 0`, `undeclared_type_role_shrink: 0`, `undeclared_evidence_type_role: 0`, `full_row_before_text_workaround: 0`, `unresolved_inline_size_budget: 0`, `unmeasured_reflow_outcome_row: 0`, `false_reflow_verification_claim: 0`이 모두 성립해야 끝난다.
