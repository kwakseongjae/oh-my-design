...
export const OMD_EXTERNAL_STAGING_ACTIVATION = "OmD native package activation: an immutable external authority controller is preregistered for this cell. The main agent is not the project owner and must never issue --reviewer/--authority-transition-approved or calculate package hashes. Bootstrap and use exactly $OMD_AUTHORITY_CONTROLLER_RUN_DIR as the mission run directory. After writing exactly one compiler-valid graph.json, provenance.json, and coverage.json draft under that run's system directory, invoke exactly once: node $OMD_AUTHORITY_CONTROLLER_EXECUTABLE . $OMD_AUTHORITY_CONTROLLER_RUN_DIR. This execution-owned controller helper and its child runtime live outside provider-writable roots; only it may prepare, externally approve, compile, checkpoint, atomically adopt, and validate the package using $OMD_AUTHORITY_CONTROLLER_RECEIPT and the cell-local $OMD_BENCH_EXTERNAL_STAGING_ROOT. If that one activation fails, stop system work and preserve the failure; never create review-v2/package-v2 or retry. Immediately reserve the remaining execution budget for the product route, including every required state and an explicit unavailable-information state.";
