...
if (!args.has("out")) { console.error("usage: prepare-multipage-cells.mjs --out <fresh-root>"); process.exit(1); }
...
for (const arm of ARMS) {
  ...
  const cell = {
    ...
    task: { id: TASK, pages: ["index.html", "events.html", "event-detail.html", "visit.html"], prompt_bytes: Buffer.byteLength(brief), prompt_sha256: sha256(brief) },
    ...
    execution_control: { serial: true, retries: 0, budget_ms: 1500000, budget_note: "25min uniformly — four pages need more room than single-page 20min; identical for every arm" },
  };
