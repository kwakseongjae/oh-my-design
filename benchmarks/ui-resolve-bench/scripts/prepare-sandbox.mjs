...
writeFileSync(join(out, instructionFile), [
  ...
  ...(deterministicReflow ? [
    ...
    `Before the single product edit, run \`node ${deterministicReflow.helper_path} source-packet .omd/reflow-closure.json\` exactly once. Write the complete proposed product to \`.omd/product-candidate.html\`, then run \`node ${deterministicReflow.helper_path} static-preview .omd/reflow-closure.json .omd/product-candidate.html\` until it passes while leaving the product and sealed artifact unchanged. Do not read or retype the passed candidate. Promote its exact bytes once with \`node ${deterministicReflow.helper_path} static-promote .omd/reflow-closure.json .omd/product-candidate.html\`, then run the same helper with \`static-close .omd/reflow-closure.json\` exactly once.`,
  ] : []),
  "",
].join("\n"), "utf8");
