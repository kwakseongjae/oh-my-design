...
const VALID_COUNTRIES = new Set(['KR','US','JP','TW','CN','UK','DE','FR','IT','FI','NL','SE','ES','LT','NO','DK','AT','EE','CZ','RO','BE','CH']);
...
const TYPES = `export interface RefEntry {
  ...
  readonly country: 'KR' | 'US' | 'JP' | 'TW' | 'CN' | 'UK' | 'DE' | 'FR' | 'IT' | 'FI' | 'NL' | 'SE' | 'ES' | 'LT' | 'NO' | 'DK' | 'AT' | 'EE' | 'CZ' | 'RO' | 'BE' | 'CH';
...
// the user actually gets on `npx oh-my-design-cli install-skills`:
//   SKILL_COUNT    = package.json "files" entries under skills/ (what npm ships;
//                    skills/ also holds local-only dirs like omd-lab-02 that are
