 ...
 * 목록 개수이기 때문).
 */
const PRIMARY_TASK_HEADING = /^\s*\d*\.?\s*(?:primary tasks?|user outcomes?)\b/i;

function primaryTaskList(segments) {
  // `heading`은 `## ` 접두사 없이 저장된다 — "13. Personas", "Primary tasks".
  // 처음에 마크다운 접두사를 기대하는 정규식을 썼다가 0건을 얻었다.
  const section = segments.find((segment) => PRIMARY_TASK_HEADING.test(String(segment.heading ?? '')));
  if (!section) return [];
  const items = [];
...
/** A dimension token is `{ value, unit }` on the wire and `6px` to a reader.
 * Without this the projected DESIGN.md printed raw JSON at the exact place a
 * person or an agent looks up a radius. */
...
/**
 * Promote the declared radius scale into typed `foundations.tokens`.
 *
 * `rounded:` is a measured token map exactly like `colors:`, and it was the
 * other half of tonight's regression: adoption turned `radius` from "6px" into
 * "" because only colours were ever promoted. Which entry becomes
 * `radius.default` follows `selectDefaultRadius` rather than a rule invented
 * here; every other entry keeps its own name and makes no claim about where it
 * applies.
 *
 * Inherits the `prose-derived` refusal from `extractColorTokens` — a scale that
 * was read out of prose is not a measurement and is not promoted.
 */
function extractRadiusTokens(frontmatter) {
  const source = frontmatter.match(/^\s{2}source:\s*(.+)$/m)?.[1]?.replace(/['"]/g, '').trim();
  if (!source || source === 'prose-derived') return {};
  const line = frontmatter.match(/^\s{2}rounded:\s*\{(.+)\}\s*$/m)?.[1];
  if (!line) return {};
  const tokens = {};
  const declared = [];
  for (const [key, raw] of Object.entries(parseInlineMap(line))) {
    const px = /^(\d+(?:\.\d+)?)(?:px)?$/.exec(raw);
    if (!px) continue;
    const value = { $type: 'dimension', $value: { value: Number(px[1]), unit: 'px' } };
    tokens[`radius.${slug(key)}`] = value;
    declared.push([key, value]);
  }
  const semantic = RADIUS_DEFAULT_SLOTS.map((slot) => declared.find(([key]) => key === slot)).find(Boolean);
  const fallback = declared.find(([key]) => !RADIUS_PILL_SLOTS.includes(key));
  const chosen = semantic ?? fallback;
  if (chosen) tokens['radius.default'] = chosen[1];
  return tokens;
}

// The UI family lives in `tokens.typography.family` as a map of named slots.
// Only one of them is the family the product interface actually renders in;
// `ui` says so outright, `sans` is the catalogue's dominant spelling for the
// same thing (229 references vs 87), and `body` is the last honest fallback.
// Exactly the order `selectUiFont` already resolves in
// (`web/src/lib/references/normalize.ts`). The readers and the migrator must
// agree on which declared family is the interface family, or a reference gets
// one face before adoption and another after. `display`, `mono`, `serif`,
// `cjk` and `fallback` are absent from that list on purpose — promoting a
// display or monospace face as the interface family is the substitution this
// project forbids.
const UI_FAMILY_SLOTS = ['ui', 'sans', 'text', 'body', 'default', 'base'];

// Likewise `selectDefaultRadius`: a named semantic key first, then the first
// key that is not a pill. 127 of the 429 references that declare `rounded` have
// no `md`, so a rule that only knew `md` would leave them with no default.
const RADIUS_DEFAULT_SLOTS = ['base', 'md', 'sm', 'lg', 'card', 'button'];
const RADIUS_PILL_SLOTS = ['full', 'pill', 'circle'];

// Roles the consumer adapter treats as the interface role, in its own priority
// order (`consumer-adapter.ts` `uiRoleIds`). The declared UI family attaches to
// whichever of these the reference actually declares — never to a heading role,
...
function parseInlineMap(text) {
  ...
  for (const part of String(text).split(',')) {
    const pair = /^\s*([A-Za-z][A-Za-z0-9_-]*)\s*:\s*(.+?)\s*$/.exec(part);
    if (!pair) continue;
    const raw = pair[2].replace(/^['"]|['"]$/g, '').trim();
    if (!raw) continue;
    out[pair[1]] = raw;
  }
  return out;
...
/**
 * Promote `tokens.typography` into typed `typography_assets.roles`.
 *
 * Why this exists: the migration mapped every typography segment into
 * `typography_assets.rules` as prose and left `roles` empty, so an adopted
 * reference projected `fontFamily: ""` even though the family was measured —
 * toss carried "Toss Product Sans" backed by 810 observed first-family uses and
 * still came out blank. Colours never had this problem because
 * `extractColorTokens` promotes them into `foundations.tokens`. This is the
 * typography half of that same promotion, and it inherits the same refusal:
 * a `prose-derived` token set is not evidence and is not promoted.
 *
 * `usage` is required by the schema and is rendered from the declared id. That
 * is a restatement of what the reference says, not a claim about what the role
 * is for — no role gets a purpose the reference did not write down.
 */
function extractTypographyRoles(frontmatter) {
  const source = frontmatter.match(/^\s{2}source:\s*(.+)$/m)?.[1]?.replace(/['"]/g, '').trim();
  if (!source || source === 'prose-derived') return [];
  const start = frontmatter.search(/^\s{2}typography:\s*$/m);
  if (start < 0) return [];
  ...
  for (const line of frontmatter.slice(start).split('\n').slice(1)) {
    ...
    const fields = parseInlineMap(rest.replace(/^\{|\}$/g, ''));
    if (key === 'family') {
      for (const slot of UI_FAMILY_SLOTS) {
        if (fields[slot]) { family = fields[slot]; break; }
      }
      continue;
    }
    const role = { id: key, usage: `declared type role: ${key}` };
    if (fields.size) role.size = /^\d+(\.\d+)?$/.test(fields.size) ? `${fields.size}px` : fields.size;
    if (fields.weight) role.weight = /^\d+$/.test(fields.weight) ? Number(fields.weight) : fields.weight;
    const lineHeight = fields.lineHeight ?? fields['line-height'] ?? fields.line_height;
    if (lineHeight) role.line_height = lineHeight;
    if (fields.tracking ?? fields.letterSpacing) role.tracking = fields.tracking ?? fields.letterSpacing;
    if (Object.keys(role).length > 2) roles.push(role);
  }
...
function buildGraphFromLegacy(markdown, inspection, options) {
  ...
  const frontmatter = frontmatterSegment?.content
    .replace(/^---\r?\n/, '')
    .replace(/\r?\n---\r?\n?$/, '') ?? '';
  const typographyRoles = frontmatter ? extractTypographyRoles(frontmatter) : [];
  const foundationTokens = frontmatter
  ...
  for (const segment of inspection.segments) {
    ...
    // Primary tasks는 `experience.primary_tasks`로 따로 나가므로 summary에 또 넣지 않는다.
    // 넣으면 렌더된 문서에 목록이 두 번 나온다 — scope 클레임 본문에 한 번(예산에 걸려
    // 잘린 채), primary-tasks 클레임에 한 번. 2026-09-17 toss 첫 패키지에서 실제로 그랬다.
    // 세그먼트는 mapped로 기록한다 — 버려진 게 아니라 다른 자리로 갔다.
    if (PRIMARY_TASK_HEADING.test(String(segment.heading ?? ''))) {
      mappedSegmentIds.push(segment.id);
      continue;
    }
  ...
  const graph = {
    ...
    foundations: {
      ...(Object.keys(foundationTokens).length ? { tokens: foundationTokens } : {}),
      ...(mapped.foundations.length ? { rules: mapped.foundations } : {}),
    },
    typography_assets: {
      ...(typographyRoles.length ? { roles: typographyRoles } : {}),
      ...(mapped['typography-assets'].length ? { rules: mapped['typography-assets'] } : {}),
    },
...
function renderTokens(tokens) {
  ...
  const lines = entries.map(([id, token]) => `- **${id}**: ${inlineCode(tokenValueText(token.$value))}${token.$description ? ` — ${token.$description}` : ''}`);
