...
const en: CliDocsDictionary = {
  ...
  overview: {
    ...
    truthBody: "The bundle contains 21 skills, 18 agent definitions, and 440 quality-graded DESIGN.md references: 141 verified_v2, 159 partial, and 140 legacy snapshots. Use verified_v2 references for public demos. Cursor 2.4+ receives 20 compatible Agent Skills plus a small project rule; separately generated specialist roles remain available on channels that support them.",
    proof: "21 product skills · 18 specialist agents · 141 verified_v2 / 159 partial / 140 legacy · zero AI calls during install",
  ...
  gettingStarted: {
    ...
    channels: [
      ...
      { name: "Cursor", body: "Installs 20 native Agent Skills, a small DESIGN.md bootstrap rule, and the shared catalog. Use `--cursor-rule-only` only for older clients.", command: "npx oh-my-design-cli@latest install-skills --agent cursor --all" },
    ],
  ...
  skills: {
    ...
    title: "21 product skills, organized by outcome.",
    lead: "Skill names are useful for explicit control, but normal product requests should trigger them naturally. The release bundle ships the 21 product skills listed below.",
    countNote: "21 product skills · 18 specialist agents",
    groups: [
  ...
  showcase: {
    ...
    previewStats: ["21 skills", "18 agents", "10 phases"],
  },
  troubleshooting: {
    eyebrow: "RECOVERY",
...
const ko: CliDocsDictionary = {
  ...
  overview: {
    ...
    truthBody: "번들은 제품 스킬 21개, 에이전트 정의 18개, 품질 등급이 표시된 DESIGN.md 레퍼런스 440개를 담습니다. 현재 verified_v2 141개, partial 159개, legacy snapshot 140개이며 공개 데모에는 verified_v2를 사용합니다. Cursor 2.4+에는 호환 Agent Skills 20개와 작은 프로젝트 규칙이 들어가며, 별도 전문 역할은 이를 지원하는 채널에만 설치됩니다.",
    proof: "제품 스킬 21개 · 전문 에이전트 18개 · verified_v2 141 / partial 159 / legacy 140 · 설치 중 AI 호출 0회",
  ...
  skills: {
    eyebrow: "기능 지도",
    title: "결과별로 묶인 제품 스킬 21개",
    lead: "전문가는 스킬 이름을 직접 지정할 수 있습니다. 보통은 만들 화면과 조건만 말하면 됩니다. 배포 패키지에는 아래 제품 스킬 21개가 포함됩니다.",
    countNote: "제품 스킬 21개 · 전문 에이전트 18개",
    groups: [
  ...
  showcase: {
    ...
    previewStats: ["스킬 21개", "에이전트 18개", "10단계"],
  },
  troubleshooting: {
    eyebrow: "복구",
...
const ja = translatedLocale(
  ...
  { overview: "概要", "getting-started": "はじめに", demo: "ライブデモ", workflows: "ワークフロー", skills: "スキル", "anti-slop": "AI slop の判断基準", showcase: "実例", troubleshooting: "トラブルシューティング", ai: "AI 向けドキュメント" },
  {
    ...
    showcase: {
      ...
      previewStats: ["スキル21個", "エージェント18個", "10フェーズ"],
    },
...
const zhCn = translatedLocale(
  ...
  { overview: "概览", "getting-started": "快速开始", demo: "现场演示", workflows: "工作流", skills: "技能", "anti-slop": "AI slop 判定", showcase: "案例", troubleshooting: "故障排查", ai: "面向 AI 的文档" },
  {
    ...
    showcase: {
      ...
      previewStats: ["21 个技能", "18 个智能体", "10 个阶段"],
    },
...
const zhTw = translatedLocale(
  ...
  { overview: "總覽", "getting-started": "快速開始", demo: "現場示範", workflows: "工作流程", skills: "技能", "anti-slop": "AI slop 判定", showcase: "案例", troubleshooting: "問題排解", ai: "給 AI 的文件" },
  {
    ...
    overview: { ...zhCn.overview, eyebrow: "給 AI 程式助理的設計脈絡", title: "讓會寫程式的助理，也能讀懂設計依據", lead: "oh-my-design 會把有品質分級的參考庫與可重複使用的設計工作流程，安裝到你正在用的程式工具。支援的工具還會加入專業角色。安裝完成後，直接說明要調整的產品介面即可。", truthTitle: "CLI 先把 AI 程式助理需要的環境準備好", truthBody: "套件包含 21 個產品技能、18 個代理定義與 440 份有品質分級的 DESIGN.md：verified_v2 141 份、partial 159 份、legacy snapshot 140 份。公開示範請使用 verified_v2。Cursor 2.4+ 會取得 20 個相容 Agent Skills 與精簡的專案規則；獨立專業角色只安裝到支援它們的通道。", proof: "21 個產品技能 · 18 個專業代理 · verified_v2 141 / partial 159 / legacy 140 · 安裝期間 0 次 AI 呼叫", outcomesTitle: "從想完成的成果開始", outcomesLead: "不用記技能名稱。說明要調整的介面與不能更動的內容，系統會選擇適合的工作流程。", outcomes: [
    ...
    skills: { ...zhCn.skills, eyebrow: "功能地圖", title: "依成果整理的 21 個產品技能", lead: "需要精準控制時可以指定技能名稱。多數情況只要說明介面與限制即可。發布套件包含下列 21 個產品技能。", countNote: "21 個產品技能 · 18 個專業代理", groups: en.skills.groups.map((g, i) => ({ ...g, title: ["建立專案設計依據", "實作產品介面", "檢查產品品質", "寫出自然的多語文案", "留下可追溯的參考"][i], description: ["建立、套用並同步專案的設計依據。", "把需求做成可操作、可交付的產品介面。", "檢查介面感受、設計一致性、模式重複、無障礙與發布準備。", "先完成韓語原稿，再針對五種語言重寫，保留相同事實與操作。", "把公開介面記錄成含版本資訊的依據，不捏造未知 token。"][i] })), routingTitle: "技能名稱可以省略", routingBody: "直接說「改善並驗證這個結帳頁」，系統會選擇完成工作所需的技能。" },
    antiSlop: {
      eyebrow: "判定方式",
    ...
    showcase: {
      ...
      previewStats: ["21 個技能", "18 個代理", "10 個階段"],
    },
