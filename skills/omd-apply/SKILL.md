...
2e. **`reflow-integrity closure`를 수행한다.** 같은 consumer route의 390px·320px·200% reflow에서 사용자에게 보이는 atomic identifier와 제품 계약이 보존하는 짧은 control/state label을 확인한다. 검사 범위는 table/list row에 한정하지 않고 같은 decision context의 선택 target, source filename, artifact ID까지 포함한다. 먼저 해당 값을 full-width reading row 또는 충분한 `minmax` 영역에 배치하고, 경쟁 column을 접은 뒤 DESIGN.md에 이미 선언된 spacing/padding 범위에서만 공간을 회수한다. 짧은 label이 compact control이나 toggle과 한 row를 공유해 선언된 line budget을 넘기면 mobile에서는 label에 full-width row를 먼저 주고 control을 다음 row 또는 별도 정렬 영역에 둔다. reflow 가능한 table/list는 mobile에서 label-above-value 또는 full-width reading cell로 바꾸는 것을 내부 horizontal scroller보다 먼저 시도한다. single atomic identifier, filename, state, short label 같은 단일 text node 자체에 `overflow-x:auto|scroll`을 두는 것은 text repair가 아니다. internal horizontal scroller는 여러 항목의 수평 관계를 보존해야 하는 실제 comparison container에만 허용한다. 의미상 수평 비교가 꼭 필요해 그 scroll region을 남기면 accessible name과 자연스러운 내부 focus target을 제공하거나 region 자체를 `tabindex="0"`으로 keyboard-reachable하게 하고 visible focus를 보존한다. 한 줄 보존은 acceptance 결과이지 첫 CSS 수단이 아니다. `white-space: nowrap`이 horizontal overflow·clipping·box collision을 만들면 사용하지 않으며, word-break·세로 글자 쌓기·숨김·복제·임의 축약·선언된 type role 미만 축소로 통과시키지 않는다. atomic identifier에는 `<wbr>`, `<br>`, U+200B zero-width space, `&shy;`/soft hyphen, CSS·JS generated break separator를 삽입하지 않는다. mobile에서 row를 stack하거나 grid를 바꾼 뒤에는 desktop의 `:nth-child` column width, `grid-area`, `flex-basis`가 selector specificity 때문에 남지 않도록 같거나 더 높은 specificity로 해제하고, 각 reading cell의 computed width가 mobile container의 전체 가용 폭을 받는지 확인한다. CSS generated mobile label은 `auto`/`max-content` track 또는 자신의 full-width row를 사용한다. fixed label track은 모든 reflow viewport에서 measured required width 이상일 때만 허용한다. browser/computed proof가 없으면 source selector specificity를 대조해 fail-close하며, break tag의 정적 존재를 성공 조건으로 주장하지 않는다. 이 교정 과정에서도 선택 target의 강조를 약화하지 않고, 하나의 decision boundary로 선언된 target·evidence·state·action을 같은 최종 DOM 경계 안에 둔다. closure는 `mid_token_fragmentation: 0`, `short_control_wrap: 0`, `generated_label_overflow: 0`, `reflow_horizontal_overflow: 0`, `reflow_clipped_control: 0`, `decision_role_escape: 0`, `target_emphasis_loss: 0`, `injected_break_opportunity: 0`, `residual_mobile_column_width: 0`, `atomic_context_wrap: 0`, `unfocusable_scroll_region: 0`, `compact_control_label_wrap: 0`, `single_text_scroller: 0`이 모두 성립해야 끝난다.
