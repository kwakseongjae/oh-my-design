...
2. **첫 편집 전 `semantic_color_ledger`를 잠근다.** 현재 화면과 계획한 변경에서 의미를 전달하는 모든 foreground/background pair를 `token`, `surface`, `content_type(normal-text|large-text|non-text)`, `contrast_proof(measured|unresolved)`로 기록한다. token 이름이나 용도가 proof는 아니다. `muted`, `secondary`, `supporting`도 normal text면 exact pair를 WCAG 상대휘도식으로 계산하고 반올림 전 값이 4.5 미만이면 실패다. measured proof가 없는 accent-on-surface pair도 의미 있는 normal text에 쓰지 않는다. 실패·미계측 normal text는 DESIGN.md의 확인된 `ink`/text-role token으로 바꾸고, accent는 인접한 non-text dot·bar·ring·icon에만 남긴다. 색만으로 상태를 구분하지 않는다. DESIGN.md에 검증 가능한 대체 token이 없으면 새 색을 만들지 않는다. `unresolved`는 위험한 pair를 유지할 허가가 아니라 fail-closed 교체 신호다.
2a. **편집 직후 `foreground closure`를 수행한다.** 최종 전달이나 선택 검증으로 넘어가기 전에 이번 diff에서 추가·변경한 foreground뿐 아니라 pre-edit ledger에서 실패·미계측으로 분류된 기존 normal-text foreground도 한 번에 닫는다. 각 항목을 `normal-text | large-text | non-text`로 다시 분류하고 실제 배경 surface를 붙인다. `accent`, `brand`, `signal`, `status`, `warning`, `muted` 같은 이름이나 DESIGN.md 설명 자체는 contrast proof가 아니다. exact pair가 반올림 전 4.5:1을 통과하지 못하면 브라우저 실행 가능 여부와 관계없이 그 텍스트를 DESIGN.md의 검증된 `ink`/text-role token으로 교체한다. accent는 같은 상태의 인접한 dot·bar·border·ring·icon·background처럼 3:1을 확인할 non-text 역할에만 남긴다. measured proof 없이 “굵어서 괜찮다”, “거의 4.5다”, “토큰에 있으니 괜찮다”고 통과시키지 않는다. 대체 text token도 없으면 장식 foreground를 제거하고 일반 텍스트로 상태명을 보존하며 새 hex를 만들지 않는다. closure 결과에 `failed_or_unresolved_normal_text_pairs: 0`이 되기 전에는 acceptance를 시작하지 않는다.
...
2e. **`reflow-integrity closure`는 아래 work packet 하나로 실행한다.** 같은 consumer route의 390px·320px·200% reflow를 검사한다. 첫 CSS 편집 전 packet의 `rows`를 DOM과 state-rendering source에서 채우고, 마지막 측정까지 행을 합치거나 지우지 않는다.
   ...
   3. **REFLOW.** 가장 좁은 조건에서 먼저 각 row의 longest atomic child와 padding/gap이 fit하는지 판단한다. sibling metadata·identifier·evidence·state 중 하나라도 줄바꿈되면 그 text를 감싸게 두지 말고 parent row를 `full-row`, 다음으로 `stack`해 각 atomic child가 자체 행을 갖게 한다. desktop의 `:nth-child` width·`grid-area`·`flex-basis`·`min-width`를 mobile cascade에서 해제하고 모든 grid/flex child에 필요한 `min-width: 0`을 둔다. compact control은 label full row 뒤에 control을 둔다. 여러 항목의 비교 관계만 named, keyboard-reachable `comparison-scroll`을 허용한다. 단일 text scroller, 숨김·복제·word-break·세로 쌓기, `<wbr>`·`<br>`·ZWSP·soft hyphen·generated separator는 금지한다. `nowrap`은 세 viewport에서 longest value가 실제 측정으로 fit하고 page overflow·clipping이 0일 때만 final selector에 사용할 수 있다.
