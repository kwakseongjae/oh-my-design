...
- pre-edit plan command가 navigation 전에 infrastructure error로 끝났다면 제품 edit 전에 `node <current-skill-dir>/scripts/reflow-artifact.mjs source-fallback-open .omd/reflow-closure.json`을 정확히 한 번 실행한다. helper가 잠근 pre-edit product sha256·inventory sha256와 `source_fallback_closure.state: opened`가 없으면 fallback edit은 금지한다. measured plan이 있거나 product가 이미 바뀐 뒤에는 이 경로를 열 수 없다.
...
- fallback을 열기 전 `static_closure_manifest`에는 target/evidence별 stable carrier selector와 row selector가 드러나는 exact required literals, 각 carrier의 exact cardinality, non-empty accessible name, `tabindex="0"`, `overflow-x:auto`, row `white-space:nowrap`, visible `:focus-visible` 계약을 넣는다. state/action hook가 carrier fragment에 들어가거나 passive row 자체에 overflow가 선언되는 source pattern은 forbidden으로 잠근다. task에 target이나 concise evidence 중 하나만 존재하면 존재하는 역할만 등록하며 없는 역할을 만들지 않는다.
- `source-fallback-open`은 존재하는 target/evidence row 각각이 `decision: comparison-scroll`이고, row 하나만 결박한 서로 다른 pre-edit carrier와 완전한 `scroll_contract`를 가졌을 때만 열린다. helper stdout의 `source_fallback_relationships`가 지정하는 `data-omd-source-fallback-carrier`, exact accessible name, `tabindex="0"`, carrier overflow, row nowrap, visible focus, 다른 decision role 배제는 권고가 아니라 `static-close`가 직접 검사하는 필수 계약이다.
