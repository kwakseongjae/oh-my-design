...
2e. **`reflow-integrity closure`를 수행한다.** 같은 consumer route의 390px·320px·200% reflow에서 사용자에게 보이는 atomic identifier와 제품 계약이 보존하는 짧은 control/state label을 확인한다. 먼저 해당 값을 full-width reading row 또는 충분한 `minmax` 영역에 배치하고, 경쟁 column을 접은 뒤 DESIGN.md에 이미 선언된 spacing/padding 범위에서만 공간을 회수한다. 한 줄 보존은 acceptance 결과이지 첫 CSS 수단이 아니다. `white-space: nowrap`이 horizontal overflow·clipping·box collision을 만들면 사용하지 않으며, word-break·세로 글자 쌓기·숨김·복제·임의 축약·선언된 type role 미만 축소로 통과시키지 않는다. atomic identifier에는 `<wbr>`, `<br>`, U+200B zero-width space, `&shy;`/soft hyphen, CSS·JS generated break separator를 삽입하지 않는다. mobile에서 row를 stack하거나 grid를 바꾼 뒤에는 desktop의 `:nth-child` column width, `grid-area`, `flex-basis`가 selector specificity 때문에 남지 않도록 같거나 더 높은 specificity로 해제하고, 각 reading cell의 computed width가 mobile container의 전체 가용 폭을 받는지 확인한다. browser/computed proof가 없으면 source selector specificity를 대조해 fail-close하며, break tag의 정적 존재를 성공 조건으로 주장하지 않는다. CSS generated mobile label도 자신의 box 안에서 겹침 없이 보여야 한다. 이 교정 과정에서도 선택 target의 강조를 약화하지 않고, 하나의 decision boundary로 선언된 target·evidence·state·action을 같은 최종 DOM 경계 안에 둔다. closure는 `mid_token_fragmentation: 0`, `short_control_wrap: 0`, `generated_label_overflow: 0`, `reflow_horizontal_overflow: 0`, `reflow_clipped_control: 0`, `decision_role_escape: 0`, `target_emphasis_loss: 0`, `injected_break_opportunity: 0`, `residual_mobile_column_width: 0`이 모두 성립해야 끝난다.
...
6. **최종 acceptance packet을 한 번 실행한다.** 같은 route에서 다음을 묶어 확인하고, 고칠 수 없는 항목은 `unresolved`로 전달한다.
   ...
   - reflow-integrity closure가 atomic identifier·short control/state label·generated mobile label과 mobile stack 이후 selector specificity를 확인했고 `mid_token_fragmentation`, `short_control_wrap`, `generated_label_overflow`, `reflow_horizontal_overflow`, `reflow_clipped_control`, `decision_role_escape`, `target_emphasis_loss`, `injected_break_opportunity`, `residual_mobile_column_width`가 모두 0
