...
2f. **`proof execution close latch`로 끝난 증명을 다시 열지 않는다.** 품질 gate는 유지하고 아래 state를 같은 consumer route의 acceptance까지 유지한다.
   ...
   - **준비된 mechanism은 `browser-harness <<'PY' … PY`다.** 사용법을 알아내기 위한 `browser-harness --help`, `browser-harness skill`, `command -v`, socket/port 탐색을 먼저 실행하지 않는다. 첫 browser-harness 호출 하나 안에서 실제 route를 열고, states와 390px·320px·200%를 순회하며 모든 registered selector의 character-range line tops·overflow·cardinality·association을 수집한다. 그 Python process가 artifact에 결과를 쓰고 `subprocess.run(["node", helper, "finalize", artifact], check=True)`까지 실행해 stdout terminal marker로 끝낸다. browser command가 반환된 뒤 `rg`/`sed`/`cat`/helper를 실행하지 않는다. 이 호출이 attach/실행 infrastructure error를 반환하면 같은 process에서 즉시 `unresolved`로 잠그며 recovery 명령을 실행하지 않는다.
