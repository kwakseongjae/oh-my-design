...
<p align="center">
  <strong>One-command bootstrap for skill-driven design with your AI coding agent.</strong> 356 real company design systems. Zero AI calls in the install. Then you just talk to your agent.
</p>
...
2. In your project, type your first prompt — copy this exactly:
   ...
   Your agent runs **`omd:init`**: it recommends a reference from the 356 real-company catalog, asks you to confirm, and writes **`DESIGN.md`** to your project root. (`omd:sync` then wires up the `CLAUDE.md` / `AGENTS.md` / Cursor shims so every agent reads it automatically.)
...
**18 skills · 16 sub-agents · 356 verified references · activation hooks** — installed by the one command above.
...
- **References** — 356 real-company `DESIGN.md` files, each verified against live sources. Every reference is also served as raw markdown at `oh-my-design.kr/<id>/design.md`, so agents can fetch it directly.
