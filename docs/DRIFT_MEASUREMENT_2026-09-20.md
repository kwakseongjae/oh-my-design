...
`unreachable` 15건 중 **12건이 HTTP 404**, 2건이 403, 1건이 네트워크 실패였다. User-Agent를 바꾸거나 재시도해도
...
**이번 실행 이후로** 모든 행이 `httpStatus`와 `userAgent`를 기록한다(커밋된 422행 중 22행만 `userAgent`를 갖는다 — 나머지는 이 필드가 생기기 전에 측정됐다). 어떤 계측기가 그 판정을 냈는지
다음 스윕이 알아야 한다.
