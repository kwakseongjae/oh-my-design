...
(표의 `tokens.components`는 **레거시 프론트매터** 기준이다. 채택된 레퍼런스에서 이 수는
사이트가 지금 서빙하는 것이 아니다 — Core v2는 컴포넌트를 구조화된 맵이 아니라
`components_states.rules` 산문으로 들고 있어서 애초에 같은 단위가 아니다. 50개 중
채택된 것은 toss 하나뿐이라 한 행에만 해당한다.)
...
| | 수 | 다음 조치 |
|---|---|---|
| 컴포넌트 로스터 확인 | **27** | 이름 단위 대조 완료 (아래) |
| 인덱스는 있으나 경로 모양이 다름 | 7 | adobe `/page/…` 114개·likelion Storybook 쿼리스트링은 **진짜 로스터** |
...
스크립트가 거기까지 못 간다고 처음에 적었는데, **URL 목록을 덤프하면 갈 수 있다.**
`--dump-urls`로 호스트별 전체 경로를 받아 마지막 경로 세그먼트를 레퍼런스 전문과 대조한다
(`web/scripts/compare-component-names.mjs`). 26건이 에이전트 26개가 아니라 표 하나가 된다.

### 손으로 확인한 6건 — 상한이 얼마나 느슨한지가 측정됐다

로스터 작업을 하면서 6개 호스트의 발행 수와 "레퍼런스가 실제로 부르는 수"를 **사람이**
확인했다. 스크립트와 나란히 놓으면 이렇다.

| | 스크립트 발행 | **실제 발행** | 스크립트 "부름" | **실제 부름** |
|---|---|---|---|---|
| wise | 56 | 55 | 5 | **0** |
| alipay | 142 | **72** | 22 | **0** |
| channeltalk | 71 | **60** | 13 | **0** |
| patternfly | 75 | 72 | 21 | **2** |
| vercel | 68 | 72 | 18 | **0** |
| zendesk | 58 | 55 | 17 | **0** |
| **합계** | 470 | **386** | **96** | **2** |

**분자가 96이 아니라 2다.** 부분 문자열 매칭이 48배 과대 계상했다. 26.6%를 "상한"이라고
적어둔 건 맞았지만, 얼마나 느슨한 상한인지는 손으로 세기 전에는 몰랐다.

분모도 틀렸다. alipay 142는 `-cn` 로케일 중복을 못 걸러서고(실제 72), channeltalk 71은
디렉터리 수인데 **패키지가 export하는 건 66, 그중 컴포넌트는 60**이다. vercel은 반대로
68이 아니라 72다 — `pill`은 `/geist/badge#pill` 앵커라 URL 슬러그로는 안 잡힌다.

유일하게 0이 아닌 patternfly의 2는 진짜다: `pf-v6-c-button pf-m-primary`와
`pf-v6-c-card pf-m-clickable`로 **호스트 자신의 클래스명에 묶여 있다.** zendesk의 3건은
`/components/button` 문서 페이지 위에서 캡쳐됐지만 클래스가 아니라 위치로 잡혀서
발행 컴포넌트라고 부르지 않는다 — 그 차이가 여기서 2와 0을 가른다.

**이 2/386을 카탈로그 전체로 외삽하면 안 된다.** 이 6건은 갭이 가장 큰 쪽에서 골랐다.
확실한 건 방향뿐이고, 방향은 A를 더 강하게 지지한다.

### 이름 단위 대조 — 발행된 컴포넌트 이름 중 레퍼런스가 부르는 것

| 레퍼런스 | 유효 이름 | 레퍼런스에 등장 | 비율 |
|---|---|---|---|
| aws-cloudscape | 1 | 0 | 0% |
| velog | 91 | 6 | 6% |
| wise | 56 | 5 | 8% |
| google | 9 | 1 | 11% |
| alipay | 153 | 23 | 15% |
| channeltalk | 71 | 13 | 18% |
| microsoft | 5 | 1 | 20% |
| uber | 77 | 16 | 20% |
| patternfly | 106 | 26 | 24% |
| vercel | 68 | 18 | 26% |
| servicenow | 15 | 4 | 26% |
| sanity | 46 | 13 | 28% |
| hashicorp | 51 | 15 | 29% |
...
| samsung | 26 | 13 | 50% |
| wanted | 4 | 2 | 50% |
| money-forward | 25 | 15 | 60% |
| yeogiotte | 6 | 4 | 66% |
| ibm | 4 | 3 | 75% |
| pega | 45 | 42 | 93% |
| **합계** | **1,206** | **321** | **26.6%** |

(표의 개별 수치는 2차 추출 기준. 1차는 URL의 **마지막** 세그먼트를 이름으로 썼는데
PatternFly의 `/components/tree-view/html`·`/design-guidelines`를 별개 컴포넌트로 셌고,
반대로 Material 3의 `/components/all-buttons`를 `accessibility`·`specs`로 만들어 google을
40개 발행인데 9개로 봤다. 2차는 **컴포넌트 디렉터리 다음** 세그먼트를 쓴다 — 이쪽도 완벽하지
않다. pega는 `/components/<범주>/<이름>` 3단이라 2차가 범주를 잡아 45/42가 11/8이 된다.
**어느 추출이 옳은지보다 중요한 건 총계가 27.7% → 26.6%로 1pp 안에서 움직였다는 것이다** —
결론은 추출 방식 선택에 안 흔들린다. yeogiotte는 2차에서 3/3으로 손 측정과 정확히 일치한다.)

**이 27.7%는 커버리지의 상한이다.** 매칭이 정규화 후 부분 문자열이라 "box button"이라고 쓴
레퍼런스가 발행된 `button`의 크레딧을 받는다. 과대 계상하는 방향이므로 **갭을 만들어낼 수는
없다** — 실제 갭은 최소한 이만큼이다.

두 부류는 "빠졌다"로 세지 않고 제외했다. **불투명 슬러그**(skyscanner `web-sEshz9Z5`,
krds `component_04_07`)는 레퍼런스가 부를 수 있는 이름이 아니다 — 1차 집계에서 skyscanner를
307개 중 0개로 만들었다. **문서 페이지**(`overview`, `guidelines`, `android`, `ios`)는
이름만으로 컴포넌트와 구분할 방법이 없어 그대로 두었고, 그래서 호스트마다 몇 개씩 남는다.

**이 측정이 의도한 것을 재는지의 검증**: 2026-09-18에 손으로 42개 이름을 적은 **pega가
45개 중 42개(93%)**로 나온다. 빠진 3개는 `overview`·`landing-pages`·`complete-work`으로
컴포넌트가 아니다. 손으로 "완전"이라 판정한 **yeogiotte는 6개 중 4개(67%)**이고 빠진 둘은
자기가 이름을 부르는 컴포넌트의 하위 페이지(`box-button-usage`·`box-button-spec`)다.
...
- **vercel** — 7,140항목 중 **`/geist/table`·`/geist/toast` 등 76개가 Geist 컴포넌트**다.
  `COMPONENT_PATH`가 0으로 셌다. 고친 방식은 adobe식 예외 추가가 아니라 의미 수정이다:
  **`ds.url`이 경로를 가지면 그 하위가 곧 디자인 시스템이다** — 세그먼트 이름이 뭐든.
  경로는 선언된 증거고 세그먼트 목록은 추측이다. 같은 수정으로 sanity 7→52, samsung 17→35.
- sendbird(1,096·블로그) · kakao(316·API 포털) · zigzag(110·기술 블로그) · hubspot(0항목).

adobe는 `ds.url`이 루트라 쓸 경로 접두사가 없고 `/page/`가 패턴에 안 걸려 판정 보류로 남는다.
한 호스트뿐이라 패턴에 넣지 않았다 — 사람이 5초면 보는 것을 위해 규칙을 넓히지 않는다.
...
**`uber/baseweb`은 `components/` 규약을 안 쓴다** — 소스가 `src/accordion`, `src/avatar`처럼
`src/` 바로 아래에 있다. 첫 규약만으로는 3개짜리 시스템으로 보고됐다. 두 번째 규약을 넣되
`src/*`는 `src/a11y`·`src/styles` 같은 비컴포넌트를 포함하므로 **어느 규약으로 셌는지 행마다
남긴다.**

**그리고 그 94개도 로스터가 아니었다**(2026-09-19 정정). 레포 트리는 가정하는 규약에 따라
3개도 94개도 나오고 **둘 다 틀렸다.** 발행 로스터는 문서 사이트가 낸다 —
`baseweb.design/components/`가 자기 10개 범주로 **89개**를 낸다(UTILITY 그룹 포함, 그래서
프로바이더와 스타일링 헬퍼가 조용히 빠지지 않고 보인다). 레포 스캔은 로스터가 아니라
**소스 레이아웃**을 잰다.
