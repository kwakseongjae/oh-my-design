...
## 모바일 뷰포트 추가 후 재측정 (2026-09-03, `render-integrity.mjs --viewports 1440x900,390x844`)

Phase 6 검토 §4.1-4에서 잡은 대로 렌더 검사에 390×844를 넣었다. 데스크톱 단독일 때 못 보던 칸이 드러난다.

| arm | 칸 | FAIL(양 뷰포트) | 모바일에서만 새로 잡힌 칸 |
|---|---:|---:|---|
| omd | 36 | 0 | 0 |
| hallmark | 36 | 3 | 0 (toss rep-2·3·4는 데스크톱에서도 ua-default) |
| uiuxpromax | 36 | 5 | 4 — coupang rep-1·rep-4, karrot rep-3, naver rep-4 (전부 overflow-x + escape) |

의미: **coupang/uiuxpromax/rep-1은 두 평가자가 재채점 끝에 합의한 확정 결함(H-OVERFLOW P1, mobile-390)과 같은 칸이다.**
기계 검사가 사람 합의 결함을 뒤늦게 독립 재현했다 — 데스크톱만 보던 이전 구성은 이 부류를 구조적으로 놓치고 있었다.
omd는 양 뷰포트 0으로 유지된다. 위 108칸 표(데스크톱 단독)와 직접 비교하지 않는다 — 측정 구성이 다르다.

부수 결함 1건: 뷰포트 인자 파싱이 `--viewports` 없을 때 첫 파일을 조용히 버렸다(`-1 + 1 = 0`).
전수 개수 점검(108 = 108)에서 잡아 고쳤다. 개수 점검 없이 「PASS」만 봤으면 못 봤을 결함이다.
