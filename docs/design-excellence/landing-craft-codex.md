...
- **LC-51 주 미디어가 화면을 지배한다.** 섹션 주 미디어 ≥35% 뷰포트 면적(섹션 중앙값), 페이지에 풀블리드(≥90%) ≥1, 폴드 주 미디어 ≥45vw 또는 ≥40vh. 여러 장은 공간이 아니라 **시간**에 쌓는다(동시 미디어 ≤6). 시험: 히어로 이미지를 빼도 첫 화면이 멀쩡하면 이미지가 약한 것이다(refero).
- **LC-52 한 화면 한 초점.** 1등 : 2등 시각 요소 면적비 ≥2:1(위반 ≤1 섹션). 동시에 움직이는 독립 모션 채널 ≤2. 섹션당 층(스크롤·유휴·호버·진입) ≤2.
- **LC-53 유휴 모션은 장식 레인에만.** T1 정보 유일성(반복 샘플) · T2 정적 대체 경로 · T3 사용자 제어(감속·정지·reduced-motion) · T4 무노동(가만히 있어도 도착) 을 전부 통과하는 요소만 자동으로 움직인다. 한 문장: 움직임이 "무엇을 볼 수 있는가"를 바꾸면 관문, "얼마나 아름다운가"만 바꾸면 장식.
- **LC-54 폴드는 5 이하.** 첫 뷰포트 의미 단위(텍스트 블록·CTA·미디어·액센트 개체) ≤5. 진입 애니 ≤1.2s 이고 H1·CTA 를 덮지 않는다. 스크롤 화살표 금지.
