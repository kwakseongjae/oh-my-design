...
| LI-34 | dominant media share | 섹션 최대 미디어 / 뷰포트: 중앙값 < 0.35 또는 페이지 최대 < 0.90 또는 폴드 < 45vw·40vh (LC-51) |
| LI-35 | hover gates | 기본 상태에서 감춰졌다가 `:hover` 로만 노출되는 콘텐츠 노드 > 0 (LC-49) |
| LI-36 | focal ratio | 섹션 1등/2등 시각 요소 면적비 < 2.0 인 섹션 > 1 (LC-52) |
| LI-37 | fold units | 첫 뷰포트 의미 단위 > 5 (LC-54) |
| LI-38 | motion channels | 섹션 동시 모션 채널 > 2 (LC-52) |
| LI-39 | hold content | 정착 뒤 홀드 구간에 읽을 텍스트/결과 상태 0 (LC-56) |
| LI-40 | whitespace chunk | 최대 연속 여백 < 0.25vh 또는 간격 값 > 3종 (LC-55) |
...
### LC-49 ~ LC-59 크기·간결·무노동 규칙 — 2026-09-04 (r3 60점, `scale-and-simplicity.md`)

r3 는 LC-1~48 과 LI-1~33 을 전부 지키고 60점을 받았다. 검사기가 **개수**만 재고(LI-24~26), 게이트가 **호버 관문을 요구**했으며(C8), 섹션당 **3층**이 허용됐기 때문이다. 아래는 그 세 구멍을 막는 규칙이다. 근거: NN/g·Baymard·WCAG 1.4.13·Laws of UX(레인 B, 21 출처), 타 스킬 10곳(레인 C), OSS 16종(레인 D), 리뷰 r3 실측.

- **LC-49 호버는 증폭이지 관문이 아니다.** 호버로만 도달하는 콘텐츠 0. 호버 상한 = 리프트·밝기 2% 또는 정지/감속. `pointer: coarse` 에서 잃는 것이 없어야 한다. (fx-library README 에만 있던 조항을 승격.)
- **LC-50 열림은 고정된다.** 펼침은 뷰포트 진입 시 자동 1회 + latch(이탈해도 안 접힘). 사용자가 여는 것은 클릭/탭, 열린 상태는 지속, 닫기는 명시적(다시 클릭·Esc).
- **LC-51 주 미디어가 화면을 지배한다.** 섹션 주 미디어 ≥35% 뷰포트 면적(섹션 중앙값), 페이지에 풀블리드(≥90%) ≥1, **폴드 히트테스트 커버리지 ≥25%**(레퍼런스 중앙 33.6 · r3 6.9; 박스 기준은 r3 를 통과시켜 기각). 여러 장은 공간이 아니라 **시간**에 쌓는다(동시 미디어 ≤6). 시험: 히어로 이미지를 빼도 첫 화면이 멀쩡하면 이미지가 약한 것이다(refero).
- **LC-52 한 화면 한 초점.** 1등 : 2등 시각 요소 면적비 ≥2:1(위반 ≤1 섹션). 동시에 움직이는 독립 모션 채널 ≤2. 섹션당 층(스크롤·유휴·호버·진입) ≤2.
- **LC-53 유휴 모션은 장식 레인에만 — 그리고 상한이다.** 화면당 유휴 애니 요소 ≤10, 폴드 무한 애니 ≤3(레퍼런스 유휴 중앙 2; r3 는 9 로 늘리고 점수가 내려갔다 — "저절로 움직임"은 wow 의 불변량이 아니다). T1 정보 유일성(반복 샘플) · T2 정적 대체 경로 · T3 사용자 제어(감속·정지·reduced-motion) · T4 무노동(가만히 있어도 도착) 을 전부 통과하는 요소만 자동으로 움직인다. 한 문장: 움직임이 "무엇을 볼 수 있는가"를 바꾸면 관문, "얼마나 아름다운가"만 바꾸면 장식.
- **LC-54 폴드는 5 이하.** 첫 뷰포트 의미 단위(텍스트 블록·CTA·미디어·액센트 개체) ≤5(말단 요소 기준으로는 ≤25 — 레퍼런스 q1; 두 정의는 다르다). 폴드 h1 ≥72px @1440 이면서 ≤7.5× body(112 상한). 진입 애니 ≤1.2s 이고 H1·CTA 를 덮지 않는다. 스크롤 화살표 금지.
- **LC-55 여백은 덩어리로.** 잉크 없는 최대 연속 세로 구간 ≥0.25vh 가 페이지에 ≥1, 섹션 간격 값 ≤3종. (LC-4 의 비율 규칙은 작은 여백을 흩뿌려도 통과한다.)
- **LC-56 홀드에는 읽을 것이 있다.** LC-48 정착 뒤 홀드 구간에 읽히는 텍스트 또는 결과 상태 1개. 길이 예산은 올리지 않는다 — 홀드는 길이가 아니라 내용의 문제다(r3: 홀드 614px 동안 텍스트 0).
- **LC-57 호버 타이밍.** in 120–160ms delay 0 / out 250–450ms delay ≥200ms. 500ms 초과 금지, 중단 가능.
- **LC-58 `hover: none` 의 기본형은 펼쳐진 상태.** 터치 기기에서 호버 층은 열린 채 렌더한다(LC-44 의 짝).
- **LC-59 밀도 다이얼을 선언한다.** 스토리보드 첫 줄에 밀도 1–10·모션 1–10. 밀도 ≤5 면 밀도 바닥(LI-24~26)보다 상한(LI-36~38)이 우선한다 — 바닥 규칙이 "최대로 채운다"로 오독되는 것을 막는다.

검사기 대응: LI-34 주 미디어 점유율 · LI-35 호버 관문 수 · LI-36 초점비 · LI-37 폴드 의미 단위 · LI-38 동시 모션 채널 · LI-39 홀드 콘텐츠 · LI-40 여백 덩어리 (§5 표).
