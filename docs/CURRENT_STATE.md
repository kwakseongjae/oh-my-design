...
- **다음 exact action:** locked order3 `neighborhood-library-landing-luna-max-r1-impeccable-prompt-only`을 Luna/max 한 번만 실행한다. terminal/hidden-image audit을 봉인하고 same-task visual·state·time/token tradeoff를 앞선 두 arm과 비교한다.
