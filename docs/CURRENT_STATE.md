...
- 기준 커밋: `355b7e85`의 1.9.822 exact 51-cell plan freeze (`codex/ui-skills-benchmark-v0`) on `ce6636c` (`main`) + npm release tag `v1.9.0`; rollback tag `checkpoint/cli-v1.9-pre-conversion-20260721`
- 갱신: 2026-08-09 · 1.9.822 pre-provider controller failure 보존 + runtime admission ordering fix 검증 완료
...
- admission ordering 수정은 focused95/95, full test906/906(3 skip), lint, build, diff-check green이다. 다음은 수정 커밋 뒤 fresh 1.9.823 catalog/plan/root/browser를 생성하고 Luna5·Terra6·Sol6 exact pair × fresh v18 task3 = 51셀을 `--max-new-cells 1`로 끝까지 실행한다.
