...
- **다음 exact action:** locked order4 `neighborhood-library-landing-luna-max-r1-ui-ux-pro-max`를 Luna/max 한 번만 실행한다. terminal/hidden-image audit을 봉인하고 same-task visual·state·time/token tradeoff를 비교한다.
