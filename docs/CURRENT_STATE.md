...
- 다음 benchmark queue는 **1.9.1 measurement kernel**이다: task-specific oracle, dirty-tree attestation, product-file-only changed 판정, normalized run exporter와 aggregate evidence package를 닫고 3 tasks×candidate/control×3 smoke를 수행한다. 이후 1.9.2에서 12개 task fixture·valid oracle·mutant calibration, 1.9.3부터 12×5 patch candidate를 순차 실행한다.
