...
- 갱신: 2026-08-09 · 1.9.819 Luna/high Reliability checkpoint 3/9 통과
- 추가 안전 설정: Cursor live 호출은 `cursor-grok-4.5-high` + 명시적 `included` 확인 없이는 spawn 전에 fail-close한다. Luna/Sol은 Codex runtime만 허용한다.

## 지금 (현재 위치)

- 1.9.819 checkpoint3 `choreographic-luna-r1-omd`는 85/85·UI-Resolved·proof/candidate exact로 통과했다. revision1·static closure1·measured browser1·recovery/duplicate/verification-after-ready0, wall140,757ms·total371,484t이다.
- 1.9.816 exact current OmD fresh task3×trial3 Reliability는 trial1의 세 태스크가 모두 통과해 3/9 complete다. retry/replacement/fallback/substitution/Cursor0, 남은 6셀은 frozen order 그대로 진행한다.
...
- 다음 허용 작업은 frozen order 넷째 셀 `entomology-luna-r2-omd` 1회 실행이다. 완료 셀은 재실행하지 않고 lifecycle hard-stop이 없을 때만 다음 셀로 진행한다.
...
- exact task bytes source commit과 별도 task-lock/admission 고정은 완료됐다. exact OmD current/Luna-high 남은 6셀을 serial·720s·pacing30s·no-retry/no-replacement/no-fallback/Cursor0으로 실행한다.
