...
- **다음 exact action:** #8 컨트롤러 이식(grok 워커 초안 → Fable 재검증 → 적대 테스트 → grok 리뷰) → schema liveness는 Luna `prepare-luna-max-admission-receipts.mjs schema-liveness` 재사용 → 새 커밋 → epoch 2 전체 재개장(lock→materialize→영수증→admit) → order1부터 재시작. 3b39dee2의 order1/2는 immutable 진단 증거, 재실행 금지.
