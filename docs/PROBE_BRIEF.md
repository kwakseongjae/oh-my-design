...
Measure only; do not edit repo files. Write full raw output to the scratchpad path given in your task and return a summary under 40 lines.

Tools: run from `web/`. Browser scripts: `import { chromium } from "playwright-core"`, launch `{headless:true, channel:"chrome", args:["--disable-blink-features=AutomationControlled"]}`, the locale given, viewport 1440×1000; waitUntil "load" + ~6s (networkidle can hang). Consent banners: reject / close non-essential only — never accept; if impossible, measure around it and say so. Logged out; never sign in, register, add to basket or submit forms. Temp scripts go in `web/scripts/.tmp-<yourtarget>-*.mjs` — delete ONLY your own temp files, never anyone else's.
