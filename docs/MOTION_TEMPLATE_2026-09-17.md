...
## 정정 — 130개가 아니라 30개다 (2026-09-17, fable advisor 검토 후)

오너가 "무비판적으로 추천대로 가고 있다"며 외부 검토를 요청했고, 두 결함이 나왔다.
둘 다 실재했다.

### 결함 1 — orphan 가드가 막으라고 만든 경우를 통과시켰다

정규식이 이랬다:

```js
/`(?:motion|ease|duration)-[a-z0-9-]+`/gi
```

이름 **뒤에 바로** 백틱이 와야 매칭된다. 그런데 산문에서 가장 흔한 형태는
`` `motion-standard / ease-enter` `` — 백틱 하나에 이름 둘이 슬래시로 묶인 것이다.
`motion-standard` 뒤에 ` / ease-enter`가 있으니 매칭되지 않는다.

그래서 **130개 중 100개가 막혀야 했는데 통과했다.** 전부 되돌렸다.
고친 정규식은 백틱 구간 전체를 보고 그 안에 토큰 이름이 있는지 묻는다.

### 결함 2 — 위치 참조가 매달렸다

`adobe`는 백틱 토큰명이 없어서 위 검사에 안 걸리는데, 표를 지우고 나니
*"(Token names and curves above are illustrative defaults …)"* 만 남아 **위가 없는
문장**이 됐다. 작업 중에 내가 이걸 보고도 "대부분 오탐"이라 판단하고 넘어갔다.
`would-dangle-positional-reference` 검사를 추가했고 `adobe`도 되돌렸다.

사실 보존 검사는 이걸 못 잡는다 — **값 손실은 보지만 의미 손실은 못 본다.**
...
|---|---:|---:|
| 기계 처리된 레퍼런스 | 130 | **30** |
| `motion_value_unsourced` | 164 | **261** |

고친 가드로 다시 돌리면 **0개**가 처리 가능하다(235개가 고아, 1개가 매달림).
즉 **이 작업은 기계적으로 되는 일이 아니다.** 거의 모든 모션 섹션이 산문에서 그 토큰
이름을 인용하고 있어서, 표를 자르려면 주변 문장을 다시 써야 한다. 30개는 그 인용이
없었던 예외다.

7월 배치가 "정리하다가 잃은" 것과 같은 실수를 한 번 반복했고, 검토로 잡혔다.
