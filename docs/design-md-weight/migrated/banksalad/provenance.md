...
- HTML-comment 2026-05-13 third-party press (Hankyung 2019-03-12, Korea Herald 2019-02-22 / 2019-08-28, PitchBook) and the founder-mission paraphrase are recorded in the source comment and are not promoted as current product narrative. The visible §11 says this reference does not rely on third-party founder mythology, funding figures, or fabricated competitive comparisons. Those press URLs and the paraphrase are not copied into portable Experience as tokens. No fictional persona names from the HTML comment are copied here (D2).
...
**Superseded synthetic §14 treatments (not verified product facts):** Empty/loading/error/success/skeleton/disabled/pressed recipes using `#434444`, `#04c584`, `#7b7b7b`, `#f5f5f5`, `--원`, 1s shimmer, 8% white highlight, `#fe493d`, `#10df99` retry, `#fd8700` stale-data bar, `#f3fdfa` success, 22px / 700 `#2b2b2b` figure, 3s toast `#2b2b2b` background, Disabled `#e1e1e1` / `#acacac`, pressed inset + ~5% darker. Portable Capture record keeps only the current default-only paragraph.

**Superseded voice-hypothesis table:** CTAs `내 카드 찾기` / `대출 한도 확인하기` / `신청하기`; headlines `이번 달 지출 분석`; recommendations `연 30만원 이상 절약될 수 있어요`; figures `1,240,000원`; success `신용점수가 855점으로 올랐어요.`; error `주민등록번호가 일치하지 않아요. 다시 확인해주세요.`; empty `아직 연동된 카드가 없어요. …`; health-asset `이번 검사로 18가지 항목을 확인할 수 있어요`. Source labels this a historical hypothesis, not official writing standard. Not rehosted as a portable voice guide.
...
- Observed 2026-07-12 voice strings and URLs are dual-destination: portable Content & Locales + this freshness/narrative ledger. “내 대출금리 조회” is Content Observed 352 only; it is not bound to the Primary-tasks reduction-request job (E2a)
