...
| `name: 강남언니` | 옮김 → H1 + Experience Scope + Content Terminology · 분리 → provenance | Byte-exact Korean at line 1, line 9, and line 235; also prov 10. Counted with `grep -oF '강남언니' <file> \| wc -l`: 5 in `DESIGN.md` (three of them inside `Gangnamunni (강남언니)`), **5** in provenance. |
...
| `primary_color: "#d54300"` | 분리 → provenance only | prov 15. The portable body carries the constraint (line 83, "no orange value is offered as a reusable UI token") and deliberately not the hex — `grep -oF '#d54300' DESIGN.md \| wc -l` = **0**, provenance = **3**. This is the source's own rule, not a new one. |
...
| `verification_v2.claims` (37 dotted paths, 5 YAML anchors) | 분리 → provenance | Claim ledger (prov 85–137), each path written out in full rather than compressed (prov 97–133), and the five anchors with their `computed-style` method and selectors at prov 89–93. Dotted paths and DOM selectors are collector metadata. Counted with `grep -oE 'tokens\.(colors\|typography\|spacing\|rounded\|components)' <file> \| wc -l` (not `grep -c`): `DESIGN.md` = **0**, `provenance.md` = **37**. `data-omd-capture` `grep -oF` = 0 in `DESIGN.md` / 11 in provenance; `home::` = 0 / 13. |
...
| `components_harvested: true` | 분리 → provenance | prov 21 and Proof notes (A1c). `grep -oF 'components_harvested' <file> \| wc -l`: 0 in `DESIGN.md`, **3** in provenance; the portable Capture record states the same fact as prose at line 144. |
...
| §1 — Cell/Welchis platform split, mission, customer perspective | 옮김 → Experience Scope (line 13) · Layout (line 216) · Terminology (line 236) | Kept as the official team's published statements. The transfer rule drawn from them is qualified at line 216. The Terminology cross-name rule at line 236 carries its own adjacent complete qualifier (B2a). The Scope proxy refusal — that neither consumer surface stands for the native app or the Welchis back office — is qualified at line 11. |
...
| §2 Color Palette & Roles (5 entries) | 옮김 → Foundations Semantic color (lines 77–81) | Role, value, and captured element preserved. Line 75 separates the observations from this contract's role naming and closes that naming with the complete B2a form. |
...
| Mid-file footer — **Verified** / Tier 1 / Tier 2 / Surface split / Conflicts | 분리 → provenance | Transcribed at prov 167–179. The Tier 2 line is quoted whole, including `https://styles.refero.design/?q=%EA%B0%95%EB%82%A8%EC%96%B8%EB%8B%88`; that percent-encoded URL is the only place the source's `95%`, `82%` and `96%` token strings occur, so the transcription is what keeps them from being a token loss. **Surface split is not wholesale dual.** `grep -oF 'Home and events are consumer-product surfaces' DESIGN.md \| wc -l` = **0** (provenance = 1). Only the second clause — `The Welchis post is documentation context only` / `its typography and controls are not Cell/product tokens` — is `DESIGN.md` = **1** at line 216, so that clause alone is the 2차 목적지 (E2a). The body's Scope sentence restates the two consumer routes in its own words and is not this footer sentence. |
...
Separate-session audit against rulebook v12 B2·B2a and E1·E2·E2a–d only. Worker report was not an input. Token values, component tables, state applicability, and section structure were not edited.

- B2a: three portable sentences that were editorial without a complete adjacent qualifier now carry one — Scope proxy refusal (line 11), Semantic role naming (line 75), Terminology cross-name rule (line 236). Body `derived editorial implementation inference` / `not Gangnamunni-authored` = **13** / **13**.
- E1: provenance had no derived-editorial inventory (0 rows vs body 10, then 13). `Portable derived-editorial scope` now enumerates the 13 body readings 1:1.
- E2: Surface split 2차 목적지를 the second clause only (`Home and events are consumer-product surfaces` is `DESIGN.md` **0**). Provenance occurrence claims for `강남언니` / `#d54300` / `components_harvested` corrected 6→5 / 4→3 / 4→3. `grep -cE` on the claims row replaced with `grep -oE … | wc -l` (dotted-path `DESIGN.md` = 0, `provenance.md` = 37).
