...
**미생존 1건의 처분.** `SAST/DAST`는 원본에서 **오직 §13 페르소나 전기 안에만** 등장한다(“Marcus Feldt … Cares that security scanning (SAST/DAST) is built into the pipeline, not bolted on”). 관측된 표면 카피가 아니라 가상 인물의 동기 서술이므로 **브랜드 발행 문자열이 아니다.** D2에 따라 §13 전체와 함께 삭제했고, 제품 능력 명칭으로 승격해 portable 본문에 되살리지 않았다 — 그것이 바로 karrot이 FAIL한 「가상 페르소나발 주장의 승격」이다. provenance `Omission ledger`의 §13 행과 이 행이 그 처분이다.
...
| §2 `#ececef` / `#a4a3a8` / `#fbfafd` (팔레트가 아니라 컴포넌트 상태 값) | 옮김 → Foundations `Semantic color` 말미 · Components `Default (Secondary) Button` · Components `State record` | 세 목적지 전부 portable 본문. A4: 이 값들을 일반 neutral role로 합치지 않고 「팔레트가 아니라 컴포넌트 상태 값」이라고 명시한 뒤 컴포넌트에 붙였다. |
...
| YAML `tokens.rounded` 4키 + §5 Border Radius Scale + §5 Grid의 `14px radius` | 옮김 → Foundations `Shape` | 5행 표로 도메인을 붙여 보존: 4px 마케팅 / 8px Pajamas / **14px** 드롭다운 nav-card 그리드 / 16px 마케팅 카드 / 160px pill. `full: 9999`는 provenance `Claim ledger`에 verbatim(doc 0회 · provenance 1회) — 원본이 그 pill 단계를 배지에서 160px로 렌더하므로 본문은 160px를 쓴다. 14px과 16px을 하나의 card radius로 합치지 않았다. |
...
| §7 Don't 8항 | 옮김 → Experience `Avoid` | 실측 9항 중 앞 8항이 원문 그대로, 이유까지 보존. **9번째는 이 이관이 추가한 범위 경계**(두 캡처를 제품 앱·핸드북·나머지 Pajamas 페이지의 권위로 쓰지 말 것)이며, 그 사실을 절 머리 문장에 명시했다 — 원본 항목 수를 늘린 것으로 읽히지 않게 하기 위함. |
...
| §9 Example Component Prompts 5개 · Iteration Guide 7항 | 삭제 + 고유값 1건은 이동(A3) | 복붙용 프롬프트 포장과 도구별 workflow는 받을 슬롯이 없으므로 삭제(산출 doc에서 `Quick Color Reference`·`Example Component Prompts`·`Iteration Guide`·`Create a Pajamas confirm button` 각 0회 실측). 다만 §9에만 있던 렌더 가능한 값 1건 — 다크 proof-stat 카드의 타이포(“Large stat in GitLab Sans 660, caption in 16px/400”; §4의 같은 카드 레코드에는 font 항목이 없다) — 은 Components `Dark Proof-Stat Card`의 `Typography` 행으로 옮겼다(실측 1회). 나머지 §9 값(8px·32px·14px·`0 12px`·4px·`#1f75cb` focus ring 등)은 §2·§3·§4에 이미 존재. |
...
| §13 Personas 3인 | **삭제 (sidecar 재수록도 안 함)** | D2. 원본 §13 머리글과 닫는 주석이 둘 다 fictional archetypes이며 이름은 illustrative라고 명시. 이름·나이·도시·전기를 산출 3파일 어디에도 남기지 않았다 — 실측: `Priya` 0회 · `Marcus` 0회 · `Ana Sousa` 0회 · `Bangalore` 0회 · `Berlin` 0회 · `Lisbon` 0회. karrot FAIL의 재발을 막기 위해 세그먼트 목록(platform engineers / DevSecOps leads / security engineers / open-source contributors)도 provenance에 재수록하지 않았고, `SAST/DAST`도 되살리지 않았다(실측 0회). Experience `Audience`에는 두 캡처 표면이 독립적으로 세우는 그룹(마케팅에서 평가하는 실무자 / Pajamas로 구현하는 빌더)만 남기고 그 그룹핑이 파생 판단임을 인접에 적었다. |
...
| B2 / B2a | 해석·인과 문장 10개 위치에 완전형 한정 인접 배치 | provenance `Derived editorial inventory` 10행이 위치별로 색인 |
...
| D2 | 가상 페르소나 승격·재수록 없음 | 이름/도시 실측 0회, 세그먼트 목록 0회 |
