...
| `typography.*.use` strings (9) | 분리 → provenance verbatim table (prov 87–95) · 그중 2건은 옮김 → Type roles Notes (lines 193, 194) | Seven of the nine are restated in the §3 Notes column in slightly different words, so the frontmatter byte form is reproduced verbatim in provenance rather than lost to a near-duplicate paraphrase (A5). Two are byte-identical in the body as well and therefore carry two destinations (E2a). Measured per file, `grep -oF … | wc -l`: `Section titles, newsroom heads` DESIGN.md 1 (line 193) / provenance 1; `Blog / news card titles` DESIGN.md 1 (line 194) / provenance 1; the other seven 0 in DESIGN.md, 1 each in provenance. |
...
| `components.*.use` strings (9) | 분리 → provenance verbatim table (prov 96–107) · 그중 1건은 옮김 → Components / Primary tasks (lines 332, 25) | Eight of the nine are paraphrased by the Role lines in Components, so the byte form is preserved in provenance. `Furiosa Access Program form field` is the exception: it survives byte-exact in the body and therefore carries two destinations (E2a). Measured, `grep -oF … | wc -l`: `Furiosa Access Program form field` DESIGN.md 2 (line 332 Role, line 25 primary task) / provenance 1; the other eight 0 in DESIGN.md, 1 each in provenance. |
...
| Legacy | Disposition | Destination / reason |
|---|---|---|
| §1 Visual Theme & Atmosphere — palette, band cadence, type description | 옮김 → Experience Scope (line 13) · Foundations Semantic color (lines 100, 101, 112, 113) · Distinctive traits (line 41) + Foundations Elevation (lines 133, 135) · Typography & Assets (line 205) | The observed interface layer. Line 13 carries nine of §1's thirteen hexes — `#000000`, `#ffffff`, `#151515`, `#111111`, `#e21500`, `#70e697`, `#fffa82`, `#cdbbff`, `#440a07` — plus `-2.1px`, 72px/84px, 16px, and the 1.6 line height. The remaining four and the shadow value went to other slots, so this row records them rather than claiming line 13 holds everything (E2a/E2c). Measured at line 13, `grep -oF … | wc -l`: `#7f7f7f` 0, `#d4d4d4` 0, `#30343b` 0, `#c0d0de` 0, `rgba(0,0,0,0.18) 0px 18px 50px` 0; they stand instead at lines 100, 101, 112, 113 and at lines 41, 133, 135. The §1 flat-chips-not-decoration reading is at line 205, qualified there. |
...
| §8 Breakpoints, Touch Targets, Collapsing Strategy, Image Behavior | 옮김 → Layout & Platforms (lines 439–457) · Image Behavior 1행은 Typography & Assets에도 (line 204) | The breakpoint table verbatim (`<640px`, `640-1024px`, `1024-1440px`) under the qualification at line 439, which states that this record attributes no computed measurement to a breakpoint. Image Behavior joins the same collapsing list because both of its clauses are responsive claims. Its product-render clause also stands in Assets, so both destinations are recorded (E2a): `grep -oF 'Product renders and chip photography' DESIGN.md | wc -l` = 2 — line 204 (Assets) and line 451 (collapsing list). |
