...
- The derived-editorial set is the following, and it is one-to-one with the body: §1 atmosphere prose together with the §11 interpretive reading (`DESIGN.md` line 15); the Primary tasks list, which no source section declares (line 21); the §1 Key Characteristics adjectives (line 45); §12 principles (line 49); §7 Do's (line 59); §7 Don'ts (line 72); §2 colour role names and the characterizations around them (line 88); §6 shadow philosophy (line 137); §15 motion rules and reduced-motion character (line 157); the font-evidence class sorting this migration introduced (line 167); the §3 Font Family character reading (line 184); §3 typography principles (line 200); the Assets reading that cards carry chips rather than decorative imagery (line 205); §14 states (line 221); §5 whitespace philosophy (line 435); §8 responsive breakpoints and collapsing rules (line 439); §10 voice reading and tone table (line 472); §10 forbidden register (line 484). None carries separate evidence attribution in the source; each is qualified adjacently in the body in the full form. Measured on `DESIGN.md` with `grep -oF … | wc -l`: `derived editorial implementation inference` 18, `FuriosaAI-authored` 18 — 18 qualifications for the 18 items above, no item without one and no qualification without an item.
