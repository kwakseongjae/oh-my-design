...
- **Proof grade (2026-08-26).** The Tier 1 proof for this reference is the adopted sibling `web/references/friendliai/.verification.md`: `## Proof — Tier 1 live inspect`, 20 raw samples, three brand-owned surfaces, dated 2026-07-02. See "Canonical proof" above. Until that adoption the migration log recorded the file as non-existent; the file was always there and the measurement (`ls` against a dotfile) was void. The correction is in `migration-log.md` § Revision 2026-08-26.
- Of the record's five unitless line heights, only the body role's `1.55` is backed by an individual raw sample (`line-height: 24.8px`). `1.10`, `1.20`, `1.30`, and `1.40` rest on the source's blanket `tokens.source: live-extract`, not on a per-element measurement. The claim-ledger rows above are left as the source states them, under this qualification.
