...
| `reference-evidence-2026-07/` | **올리지 않음** | 위 트리의 2026-09-17 동결본 177개, 145MB. 읽기 전용 |
| `reverify/` | **올리지 않음** | 재검증 큐·태스크 패킷 333개. 재생성 가능 |
| `reference-create/` | 커밋됨 | CREATE 실행 기록(packet/run). 영구 학습 산출물이라 남긴다 |
| `local-store.manifest.json` | 커밋됨 | 위 세 로컬 트리의 **보관 증명**(687파일 / 291.0MB) |
| `README.md` | 커밋됨 | 이 문서 |
...
### 왜 `-2026-07` 동결본을 따로 두는가 (2026-09-17)

현재 140개 verified 클레임은 **2026-07-11~14에 관측된 것**이고, 그 관측의 원본이
`reference-evidence/`다. 10월 재캡쳐는 같은 파일명(`<id>.json`)에 덮어쓴다 — 즉 복구
작업을 시작하는 순간, 지금 출하 중인 주장의 근거가 사라진다.

그러면 "7월 배치가 무엇을 보고 그렇게 썼는가"를 더는 물을 수 없게 된다. 7월 배치가
컴포넌트를 지워서 게이트를 통과했다는 사실을 **찾아낼 수 있었던 것도** 번들이 남아
있었기 때문이다. 감사 가능성을 재캡쳐와 맞바꾸지 않으려고 동결본을 만든다.

`chmod a-w`로 쓰기를 막아 두었다. 재캡쳐는 `reference-evidence/`에만 쓴다.
