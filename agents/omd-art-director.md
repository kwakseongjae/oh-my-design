...
코덱스의 밀도 규칙은 비율·범위라서 **적게 담아도 통과한다.** 첫 stripe 랜딩은 길이·섹션 높이 규칙을 지키면서
본문 12화면 중 7화면이 잉크 12% 미만이었다. 그래서 스토리보드의 각 섹션 행에 다음을 **개수**로 적는다.

- 그 섹션의 미디어 개수(페이지 전체로 **vh당 1개 이상**), 폴드 섹션은 **3개 이상**
- 가로 스크롤러를 쓰면 항목 **8개 이상**
- 페이지 전체에 **모션 에셋 1개 이상**(6초 루프, reduced-motion 정지 이미지 대체)
- 섹션 잉크 목표 **26% 이상** — 못 채우면 패딩을 줄이거나 에셋을 늘린다
