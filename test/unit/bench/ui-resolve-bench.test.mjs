...
describe("UI-Resolve Bench sandbox preparation", () => {
  ...
  it("preregisters the museum desktop decision-context replacement without provider exposure", () => {
    const result = JSON.parse(readFileSync(join(
      repoRoot,
      "benchmarks/ui-resolve-bench/reports/museum-desktop-decision-context-luna-1.9.609/PREREGISTRATION.json",
    ), "utf8"));
    expect(result).toMatchObject({
      product_version: "1.9.609",
      status: "PREREGISTERED_PROVIDER_ZERO",
      provider_calls: 0,
      tokens_to_target: { attempt_order: 46, prior_observed_provider_tokens_minimum: 101844158, prior_usage_unavailable_cells: 6 },
      task: { id: "museum-loan-crate-release-v0.1", source_commit: "e77c3d8ae959078b3cb51f5ea1960b8a30f14dcc", model_exposures: 0 },
      sources: {
        control: { source_commit: "5eaa26c9739a9e697f4707b06c31a32773706c16", variant_id: "omd-decision-context-closure-candidate" },
        candidate: { source_commit: "e8a6f083f5cb278d18d1ab812b7d154a486a8c1b", variant_id: "omd-desktop-decision-context-candidate" },
      },
      isolated_delta_contract: { candidate_adds_desktop_final_product_measurement: true, candidate_removes_missing_observation_default_pass: true, candidate_adds_nested_registered_carrier_pre_edit_rejection: true },
      matrix: { model_id: "gpt-5.6-luna", effort: "high", scheduled_cells: 6, timeout_seconds: 900, inter_cell_delay_seconds: 120, retry_policy: "none-primary" },
      promotion_gates: { candidate_ui_resolved_trials_required: 3, candidate_proof_compliant_trials_required: 3, candidate_desktop_decision_context_observed_trials_required: 3, candidate_nested_carrier_inventory_closed_trials_required: 3 },
    });
  });
  ...
  it("baselines a fresh unseen runway-lighting decision task before model exposure", () => {
    const result = JSON.parse(readFileSync(join(
      repoRoot,
      "benchmarks/ui-resolve-bench/reports/runway-lighting-task-baseline-1.9.617/SUMMARY.final.json",
    ), "utf8"));
    expect(result).toMatchObject({
      product_version: "1.9.617",
  ...
  it("baselines a fresh unseen rail-signal decision task before model exposure", () => {
    const result = JSON.parse(readFileSync(join(
      repoRoot,
      "benchmarks/ui-resolve-bench/reports/rail-signal-task-baseline-1.9.626/SUMMARY.final.json",
    ), "utf8"));
    expect(result).toMatchObject({
      product_version: "1.9.626",
      task_id: "rail-signal-speed-restriction-release-v0.1",
      status: "FRESH_UNSEEN_TASK_BASELINED",
      provider_calls: 0,
      model_exposures: 0,
      task_tree_sha256: "474d984e11a7cc3d2496e63399f61ff12b75d45a136c3ddb80cd01ebf60d15e5",
      baseline: {
        variant_id: "raw-design-md",
        score_sha256: "c3bf6115094ca92178b448c882ccca85f53ba1d158e947dd4558d06ac90e5b1f",
        objective_score: 75,
        objective_max: 85,
        automated_gate_pass: false,
        critical_gates: {
      ...
      locked_facts: {
        signal_assets: 4,
        inspection_records: 6,
        possession_windows: 2,
        view_controls: 3,
        protected_hook_mismatches: 0,
      },
      browser_harness_attestation: {
        connection_name: "bench19366",
        attempts: 1,
  ...
  it("prepares six equal untouched rail-signal cells from exact detached sources", () => {
    const result = JSON.parse(readFileSync(join(
      repoRoot,
      "benchmarks/ui-resolve-bench/reports/rail-signal-plan-bootstrap-luna-1.9.628/PREPARATION.json",
    ), "utf8"));
    expect(result).toMatchObject({
      product_version: "1.9.630",
      status: "PREPARED_PROVIDER_ZERO",
      provider_calls: 0,
      scheduled_cells: 6,
      prepared_cells: 6,
      ...
      isolated_delta_guardrail: {
        shared_context_client_width: true,
        shared_carrier_offset_width: true,
        control_requires_external_snapshot_before_runner: true,
        candidate_bootstraps_missing_snapshot_before_navigation: true,
        candidate_emits_zero_attempt_marker_for_pre_navigation_validation: true,
        candidate_blocks_product_edit_until_plan_close_and_guardrails: true,
        all_three_control_cells_match: true,
        all_three_candidate_cells_match: true,
      },
      execution_contract: {
        next_cell: "luna-rail-r1-control",
        max_new_cells: 1,
        retry: false,
        inter_cell_delay_seconds: 120,
      },
  ...
  it("pins the unseen airworthiness task before any provider exposure", () => {
    const result = JSON.parse(readFileSync(join(
      repoRoot,
      "benchmarks/ui-resolve-bench/reports/airworthiness-task-pin-1.9.645/TASK-PIN.json",
    ), "utf8"));
    expect(result).toMatchObject({
      product_version: "1.9.645",
      status: "TASK_PINNED_PROVIDER_ZERO",
      provider_calls: 0,
      model_exposures: 0,
      task: {
        id: "airworthiness-release-review-v0.1",
        source_commit: "7a6ea21a46ae0537f259d03293d66f74327faf7a",
        git_task_tree: "6881d7810d5587cea82e5f938c69b4a083b81504",
        portable_task_tree_sha256: "124ca2c48fd6bca581a110ed4229ce1c4fc983aa448a193fbffdef3c0f8e1482",
        baseline_score_sha256: "5d59ea7cdace45142d42329d8e72cce7987a39a959a1d7ed2689b3988fbecf2c",
      },
      mutation_policy: {
        task_mutation_after_pin: false,
        prompt_mutation_after_pin: false,
        starter_mutation_after_pin: false,
        evaluator_contract_mutation_after_pin: false,
      },
    });
  });
