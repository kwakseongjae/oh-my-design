...
describe("omd:apply delivery contract", () => {
  ...
  it("locks observable behavior before visual expansion", () => {
    ...
    expect(skill).toContain("첫 CSS 편집 전에 `reflow scope manifest`를 고정한다");
    expect(skill).toContain("DOM과 state-rendering source에서 전수한다");
    expect(skill).toContain("JS render 함수·template literal·state map");
    expect(skill).toContain("dynamic state row로 반드시 포함한다");
    expect(skill).toContain("paired toggle/button/select를 이름 붙이는 관계를 확인해 별도 row");
    expect(skill).toContain("`origin(static|dynamic)`");
    expect(skill).toContain("`current_controlling_selector`, `planned_final_selector`");
    expect(skill).toContain("outcome table과 identity/cardinality를 1:1 대조한다");
    expect(skill).toContain("type role을 먼저 확정한다");
    expect(skill).toContain("body/heading 상속값으로 두지 않고 선언된 label role을 적용한다");
    expect(skill).toContain("compact control label은 element tag가 아니라 같은 control group");
    expect(skill).toContain("paired toggle/button/select의 의미를 이름 붙이는 짧은 visible copy");
    expect(skill).toContain("source가 `label`, `strong`, `span`, `p` 중 무엇이든");
    ...
    expect(skill).toContain("unclassified_compact_control_copy: 0");
    expect(skill).toContain("selected target, source filename, artifact ID와 다른 atomic identifier는 evidence/metadata가 아니며");
    ...
    expect(skill).toContain("pre-edit scope manifest의 모든 identity를 같은 cardinality로 가져와");
    expect(skill).toContain("`final_controlling_selector`");
    expect(skill).toContain("manifest와 outcome identity set 또는 row count가 다르면");
