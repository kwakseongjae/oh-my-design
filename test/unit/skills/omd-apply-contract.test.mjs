...
describe("omd:apply delivery contract", () => {
  ...
  it("runs reflow through one compact, measurable work packet", () => {
    ...
    expect(skill).toContain("그 전에 일부 확인용 source command를 실행하면 범위와 결과에 관계없이 이미 static budget을 소비한 것");
  ...
  it("operationalizes proof as a revision-bound close latch", () => {
    ...
    expect(skill).toContain("검사를 반복해 green으로 꾸미지 않는다");
    ...
    expect(skill).toContain("task 전체 consolidated static closure 1회 + task 전체 browser-harness command 1회");
